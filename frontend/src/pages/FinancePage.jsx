...
export default function FinancePage() {
    ...
    const availableTabs = [
        ...(canSeePayments ? [{ key: "payments", label: "To'lovlar", icon: Wallet }] : []),
        ...(canManageExpenses ? [{ key: "expenses", label: "Xarajatlar", icon: TrendingDown }] : []),
        ...(canSeeReports ? [{ key: "payroll", label: "Maoshlar", icon: Banknote }] : []),
        ...(canSeeReports ? [{ key: "report", label: "Hisobot", icon: BarChart3 }] : []),
    ];
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {availableTabs.length === 0 ? (<Card className="py-16 text-center text-[13px] text-gray-400">Sizga hech qanday moliyaviy ruxsat berilmagan.</Card>) : (<>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-1 bg-gray-100 dark:bg-white/5 rounded-xl p-1 overflow-x-auto">
              {availableTabs.map((t) => {
                const Icon = t.icon;
                return (<button key={t.key} onClick={() => setTab(t.key)} className={cn("flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-[13px] font-medium transition-colors whitespace-nowrap", tab === t.key ? "bg-white dark:bg-gray-800 text-[#5E2CA5] shadow-sm" : "text-gray-500 dark:text-gray-400")}>
...
function PaymentsTab({ month, canManage }) {
    ...
    const load = useCallback(async () => {
        ...
        setError(false);
        try {
            const res = await apiFetch(`/api/payments?month=${month}&limit=100`);
            if (!res.ok) {
                setError(true);
                setLoading(false);
                return;
    ...
    useEffect(() => {
        ...
        const t = setTimeout(async () => {
            const res = await apiFetch(`/api/leads?search=${encodeURIComponent(q)}&limit=8`);
            if (res.ok) {
                const data = await res.json();
    ...
    return (<div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        ...
        {canManage && (<button onClick={() => setShowAdd(true)} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-medium bg-[#5E2CA5] hover:bg-[#4a2280] text-white shadow-sm transition-colors">
            <Plus className="w-4 h-4"/> To&apos;lov qo&apos;shish
          </button>)}
      </div>

      <Card className="overflow-hidden">
        {loading ? (<div className="py-16 text-center text-[13px] text-gray-400 animate-pulse">Yuklanmoqda...</div>) : error ? (<div className="py-16 text-center text-[13px] text-red-500">Ma&apos;lumotni yuklab bo&apos;lmadi. Sahifani yangilab ko&apos;ring.</div>) : payments.length === 0 ? (<div className="py-16 text-center text-[13px] text-gray-400">Bu oyda to&apos;lov yo&apos;q</div>) : (<div className="divide-y divide-gray-50 dark:divide-white/5">
            {payments.map((p) => (<div key={p.id} className="flex items-center gap-3 px-5 py-3">
      ...
      {showAdd && (<Modal title="To'lov qo'shish" onClose={() => setShowAdd(false)} footer={<>
          ...
          <div>
            ...
            {selectedLead ? (<div className="flex items-center justify-between px-3 py-2 rounded-xl border border-[#5E2CA5]/40 bg-[#5E2CA5]/5 text-[13px]">
                ...
                <input value={leadQuery} onChange={(e) => setLeadQuery(e.target.value)} placeholder="Ism yoki telefon..." className={fieldCls}/>
                {leadOptions.length > 0 && (<div className="absolute left-0 right-0 top-full mt-1 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-white/10 z-10 max-h-48 overflow-y-auto">
                    {leadOptions.map((l) => (<button key={l.id} type="button" onClick={() => { setSelectedLead(l); setLeadOptions([]); }} className="w-full text-left px-3 py-2 text-[13px] hover:bg-gray-50 dark:hover:bg-white/5">
...
function ExpensesTab({ month, canManage }) {
    ...
    const load = useCallback(async () => {
        ...
        setError(false);
        try {
            const res = await apiFetch(`/api/expenses?month=${month}`);
            if (!res.ok) {
                setError(true);
                setLoading(false);
                return;
    ...
    return (<div className="space-y-3">
      ...
      <Card className="overflow-hidden">
        {loading ? (<div className="py-16 text-center text-[13px] text-gray-400 animate-pulse">Yuklanmoqda...</div>) : error ? (<div className="py-16 text-center text-[13px] text-red-500">Ma&apos;lumotni yuklab bo&apos;lmadi. Sahifani yangilab ko&apos;ring.</div>) : expenses.length === 0 ? (<div className="py-16 text-center text-[13px] text-gray-400">Bu oyda xarajat yo&apos;q</div>) : (<div className="divide-y divide-gray-50 dark:divide-white/5">
            {expenses.map((e) => (<div key={e.id} className="flex items-center justify-between px-5 py-3">
                <div>
      ...
      {showAdd && (<Modal title="Xarajat qo'shish" onClose={() => setShowAdd(false)} footer={<>
...
function PayrollTab({ month }) {
    ...
    return (<div className="space-y-3">
      <span className="text-[13px] text-gray-500 dark:text-gray-400">Jami: <strong className="text-gray-900 dark:text-white">{total.toLocaleString("ru-RU")} so&apos;m</strong></span>
      <Card className="overflow-hidden">
        {loading ? (<div className="py-16 text-center text-[13px] text-gray-400 animate-pulse">Yuklanmoqda...</div>) : error ? (<div className="py-16 text-center text-[13px] text-red-500">Ma&apos;lumotni yuklab bo&apos;lmadi. Sahifani yangilab ko&apos;ring.</div>) : byUser.length === 0 ? (<div className="py-16 text-center text-[13px] text-gray-400">Bu oyda maosh yozuvi yo&apos;q</div>) : (<div className="divide-y divide-gray-50 dark:divide-white/5">
            {byUser.map((u) => (<div key={u.userId} className="flex items-center gap-3 px-5 py-3">
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-[11px] font-bold flex-shrink-0" style={{ backgroundColor: "#5E2CA5" }}>
                  {getInitials(u.name)}
                </div>
                <div className="flex-1 min-w-0">
...
function ReportTab({ month }) {
    ...
    useEffect(() => {
        setLoading(true);
        setError(false);
        apiFetch(`/api/reports/financial?month=${month}`)
            .then(async (res) => {
            if (!res.ok) {
                setError(true);
                return;
    ...
    if (loading)
        return <Card className="py-16 text-center text-[13px] text-gray-400 animate-pulse">Yuklanmoqda...</Card>;
    if (error || !report)
        return <Card className="py-16 text-center text-[13px] text-red-500">Ma&apos;lumotni yuklab bo&apos;lmadi. Sahifani yangilab ko&apos;ring.</Card>;
    const rows = [
    ...
    return (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {rows.map((r) => (<Card key={r.label} className="p-5">
