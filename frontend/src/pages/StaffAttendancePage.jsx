...
export default function StaffAttendancePage() {
    ...
    const load = useCallback(async () => {
        setLoading(true);
        const res = await apiFetch(`/api/staff-attendance?date=${date}`);
        if (res.ok) {
    ...
    const checkIn = async (userId) => {
        ...
        setBusyId(null);
        if (res.ok) {
            toast.success("Kelgani belgilandi");
            load();
        }
        else {
    ...
    const checkOut = async (userId) => {
        ...
        setBusyId(null);
        if (res.ok) {
            toast.success("Ketgani belgilandi");
            load();
        }
        else {
            const e = await res.json().catch(() => ({}));
    ...
    const filtered = useMemo(() => {
        if (tab === "teacher")
            return staff.filter((s) => s.isTeacher);
        if (tab === "staff")
            return staff.filter((s) => !s.isTeacher);
        return staff;
    }, [staff, tab]);
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <DatePicker value={date} onChange={(e) => setDate(e.target.value)} className="px-3 py-2 text-[13px]"/>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
        {KPI_CARDS.map((c) => {
            const Icon = c.icon;
            return (<div key={c.label} className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-4 flex items-center gap-3">
      ...
      <div className="flex items-center gap-1 bg-white dark:bg-gray-900 rounded-xl p-1 shadow-sm w-fit">
        {TABS.map((t) => (<button key={t.key} onClick={() => setTab(t.key)} className={cn("px-4 py-1.5 rounded-lg text-[13px] font-medium transition-colors", tab === t.key
        ...
        {loading ? (<div className="py-16 text-center text-[13px] text-gray-400 animate-pulse">Yuklanmoqda...</div>) : filtered.length === 0 ? (<div className="py-16 text-center">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                {filtered.map((s) => (<tr key={s.id} className="hover:bg-gray-50/60 dark:hover:bg-white/5 transition-colors duration-100">
                    {/* F.I. */}
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[11px] text-gray-400 truncate">
                            {s.isTeacher ? (s.subject || "O'qituvchi") : ROLE_LABELS[s.role] || s.role}
                          </p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        {s.status === "late" && (<span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-400">
                            Kech
                          </span>)}
                        {s.status === "ontime" && (<span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-400">
                            O&apos;z vaqtida
                          </span>)}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      {!s.checkIn ? (<button onClick={() => checkIn(s.id)} disabled={busyId === s.id} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[12px] font-semibold bg-[#5E2CA5] hover:bg-[#4a2280] text-white transition-colors disabled:opacity-60 whitespace-nowrap">
                          <LogIn className="w-3.5 h-3.5"/>
                          Keldi
                        </button>) : !s.checkOut ? (<button onClick={() => checkOut(s.id)} disabled={busyId === s.id} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[12px] font-semibold bg-gray-100 dark:bg-white/10 hover:bg-gray-200 dark:hover:bg-white/20 text-gray-700 dark:text-gray-200 transition-colors disabled:opacity-60 whitespace-nowrap">
                          <LogOut className="w-3.5 h-3.5"/>
                          Ketdi
                        </button>) : (<span className="inline-flex items-center gap-1 text-[12px] font-medium text-emerald-600 dark:text-emerald-400 whitespace-nowrap">
