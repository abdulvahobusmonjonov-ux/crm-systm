...
export default function ExercisesTab({ groupId, month }) {
    ...
    const load = useCallback(async () => {
        ...
        setError(false);
        try {
            const res = await apiFetch(`/api/teacher/groups/${groupId}/exercises?month=${month}`);
            if (!res.ok) {
                setError(true);
                setLoading(false);
                return;
    ...
    const createExercise = async () => {
        if (!form.date || !form.title.trim()) {
            toast.error("Sana va nomni kiriting");
            return;
        }
        setCreating(true);
        const res = await apiFetch(`/api/teacher/groups/${groupId}/exercises`, {
    ...
    const saveScore = async (exerciseId, leadId, value) => {
        ...
        if (score === null || Number.isNaN(score))
            return;
        const res = await apiFetch(`/api/teacher/groups/${groupId}/exercises/${exerciseId}/scores`, {
            method: "POST", headers: { "Content-Type": "application/json" },
    ...
    return (<div className="space-y-3">
      {canEdit && (<div className="flex items-center justify-end">
          <button onClick={() => { setForm({ date: "", title: "" }); setShowAdd(true); }} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-medium bg-[#5E2CA5] hover:bg-[#4a2280] text-white shadow-sm transition-colors">
            <Plus className="w-4 h-4"/> Mashq qo&apos;shish
          </button>
        </div>)}

      <Card className="overflow-hidden">
        {loading ? (<div className="py-16 text-center text-[13px] text-gray-400 animate-pulse">Yuklanmoqda...</div>) : error ? (<div className="py-16 text-center text-[13px] text-red-500">Ma&apos;lumotni yuklab bo&apos;lmadi. Sahifani yangilab ko&apos;ring.</div>) : students.length === 0 ? (<div className="py-16 text-center text-[13px] text-gray-400">Bu guruhda o&apos;quvchi yo&apos;q</div>) : exercises.length === 0 ? (<div className="py-16 text-center text-[13px] text-gray-400">Bu oyda mashq qo&apos;shilmagan</div>) : (<div className="overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-gray-100 dark:border-white/5">
                  ...
                  {exercises.map((ex) => (<th key={ex.id} className="px-2 py-3 text-center text-[11px] font-medium text-gray-500 dark:text-gray-400 min-w-[90px]" title={ex.title}>
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                {students.map((s, rowIdx) => (<tr key={s.leadId} className={cn("hover:bg-gray-50/80 dark:hover:bg-white/5 transition-colors duration-100", rowIdx % 2 === 1 && "bg-gray-50/30 dark:bg-white/[0.02]")}>
                    ...
                    return (<td key={ex.id} className="px-1 py-2 text-center">
                          {isEditing ? (<input autoFocus type="number" min={0} max={100} defaultValue={value ?? ""} onBlur={(e) => { saveScore(ex.id, s.leadId, e.target.value); setEditing(null); }} onKeyDown={(e) => { if (e.key === "Enter")
                            e.target.blur(); if (e.key === "Escape")
                            setEditing(null); }} className="w-9 h-7 text-center text-[12px] rounded-lg border border-[#5E2CA5]/40 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30"/>) : (<button onClick={() => canEdit && setEditing({ exerciseId: ex.id, leadId: s.leadId })} disabled={!canEdit} className={cn("w-9 h-7 rounded-lg mx-auto flex items-center justify-center text-[12px] font-semibold transition-colors", !canEdit && "cursor-not-allowed opacity-80", typeof value === "number"
      ...
      {showAdd && (<Modal title="Yangi mashq" onClose={() => setShowAdd(false)} footer={<>
          ...
          <div>
            ...
            <input value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} placeholder="Masalan: 3-dars mashqi" className={fieldCls}/>
          </div>
        </Modal>)}
    </div>);
}
