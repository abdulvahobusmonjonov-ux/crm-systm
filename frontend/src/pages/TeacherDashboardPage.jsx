      ...
      <Card className="overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 dark:border-white/5 flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-1 bg-gray-100 dark:bg-white/5 rounded-xl p-1">
            {["all", "today"].map((f) => (<button key={f} onClick={() => setFilter(f)} className={cn("px-3 py-1.5 rounded-lg text-[12px] font-medium transition-colors", filter === f ? "bg-white dark:bg-gray-800 text-[#5E2CA5] shadow-sm" : "text-gray-500 dark:text-gray-400")}>
        ...
        {loading ? (<div className="p-5 space-y-2">
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {filtered.map((g) => (<tr key={g.id} className="border-b last:border-0 border-gray-50 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-5 py-3.5 hidden sm:table-cell text-[13px] text-gray-500 dark:text-gray-400">
                      {g.room ? (<span className="inline-flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-gray-400"/>{g.room}</span>) : "—"}
                    </td>
                  </tr>))}
              </tbody>
            </table>
