...
export default function TeacherGroupPage() {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        setError(false);
        apiFetch(`/api/teacher/groups/${groupId}`)
            .then(async (res) => {
            if (!res.ok) {
                if (!cancelled)
                    setError(true);
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          {group && (<div className="flex items-center gap-3 mt-0.5 text-[13px] text-gray-400 dark:text-gray-500">
      ...
      {!loading && error ? (<div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm py-16 text-center text-[13px] text-red-500">
          Ma&apos;lumotni yuklab bo&apos;lmadi. Sahifani yangilab ko&apos;ring.
        </div>) : !loading && !group ? (<div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm py-16 text-center text-[13px] text-gray-400">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-1 bg-gray-100 dark:bg-white/5 rounded-xl p-1 overflow-x-auto">
              {TABS.map((t) => (<button key={t.key} onClick={() => setTab(t.key)} className={cn("px-3.5 py-2 rounded-lg text-[13px] font-medium transition-colors whitespace-nowrap", tab === t.key ? "bg-white dark:bg-gray-800 text-[#5E2CA5] shadow-sm" : "text-gray-500 dark:text-gray-400")}>
                  {t.label}
                </button>))}
            </div>
