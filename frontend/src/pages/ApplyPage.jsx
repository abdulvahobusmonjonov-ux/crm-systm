...
export default function ApplyPage() {
    const [cfg, setCfg] = useState({ courses: [], name: "Robocode CRM", logo: "" });
    ...
    useEffect(() => {
        apiFetch("/api/public/lead").then((r) => r.json()).then(setCfg).catch(() => { });
    }, []);
    const submit = async (e) => {
        ...
        if (form.referrerPhone && !isPhoneComplete(form.referrerPhone)) {
            setError("Tavsiya qilgan kishining telefon raqamini to'liq kiriting");
            return;
        }
        setLoading(true);
        const res = await apiFetch("/api/public/lead", {
        ...
        if (res.ok)
            ...
            setError(d.error || "Xatolik. Qayta urinib ko'ring.");
        }
    };
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-[#5E2CA5]/5 via-white to-[#5E2CA5]/10 flex items-center justify-center p-4 sm:p-6" style={{ fontFamily: "'Inter', sans-serif" }}>
      <div className="w-full max-w-md">
        <div className="text-center mb-6">
          <img src={cfg.logo || LOGO_URL} alt={cfg.name} className="inline-block w-16 h-16 rounded-2xl mb-3 shadow-lg object-contain bg-white"/>
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 sm:p-8">
          {done ? (<div className="text-center py-6 sm:py-8">
              ...
              <button onClick={() => { setDone(false); setForm({ fullName: "", phone: "", courseId: "", preferredDays: "", lessonTime: "", note: "", referrerName: "", referrerPhone: "", website: "" }); }} className="mt-6 text-[#5E2CA5] text-sm font-semibold hover:underline">
              ...
              <button type="submit" disabled={loading} className="w-full py-3 bg-[#5E2CA5] hover:bg-[#4e2488] disabled:opacity-60 text-white font-semibold rounded-xl shadow-sm transition-colors">
