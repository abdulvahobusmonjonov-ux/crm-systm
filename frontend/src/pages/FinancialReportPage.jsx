...
export default function FinancialReportPage() {
    ...
    const load = useCallback(async () => {
        setLoading(true);
        const res = await apiFetch(`/api/reports/financial?month=${month}`);
        if (res.ok)
    ...
    const exportCSV = async () => {
        const res = await apiFetch(`/api/reports/financial?month=${month}&format=csv`);
        if (!res.ok)
            return;
        ...
        a.href = url;
        a.download = `moliyaviy_hisobot_${month}.csv`;
        a.click();
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5 print:bg-white print:p-0" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 print:px-6 print:pt-6">
        ...
        <div className="flex items-center gap-2 flex-wrap print:hidden">
          <MonthPicker value={month} onChange={e => setMonth(e.target.value)} className="px-3 py-2 text-[13px]"/>
          <button onClick={exportCSV} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-medium bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
            <Download className="w-4 h-4"/> Excelga eksport
          </button>
          <button onClick={printPDF} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-medium bg-[#5E2CA5] hover:bg-[#4a2280] text-white shadow-sm transition-colors">
      ...
      {loading ? (<div className="space-y-5">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl h-72 animate-pulse"/>
        </div>) : !data ? (<div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm py-20 text-center text-[13px] text-gray-400">
          Ma&apos;lumot yo&apos;q
        </div>) : (<>
          {/* KPI cards */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 print:grid-cols-5 print:px-6">
            {kpis.map(k => {
                const Icon = k.icon;
                return (<div key={k.label} className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-5 print:shadow-none print:border print:border-gray-200">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-5 print:shadow-none print:border print:border-gray-200 print:break-inside-avoid print:px-6">
            <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
              ...
              <div className="flex items-center gap-4 text-[12px] text-gray-500 dark:text-gray-400">
                {["revenue", "expense", "payroll"].map(t => (<span key={t} className="flex items-center gap-1.5">
                    <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: dark ? TYPE_COLOR[t].dark : TYPE_COLOR[t].light }}/>
                    {TYPE_COLOR[t].label}
                  </span>))}
              </div>
            </div>
            {data.categories.length === 0 ? (<div className="py-14 text-center text-[13px] text-gray-400">Bu oy uchun ma&apos;lumot yo&apos;q</div>) : (<ResponsiveContainer width="100%" height={Math.max(240, data.categories.length * 36)}>
                <BarChart data={data.categories} layout="vertical" margin={{ left: 8, right: 24 }}>
                  ...
                  <Tooltip formatter={(value) => formatCurrency(Number(value ?? 0))} contentStyle={{ borderRadius: 12, border: "1px solid #f1f1f5", fontSize: 12 }} cursor={{ fill: BRAND, fillOpacity: 0.06 }}/>
                  <Bar dataKey="amount" radius={[0, 8, 8, 0]} maxBarSize={22}>
                    {data.categories.map((c, i) => (<Cell key={i} fill={dark ? TYPE_COLOR[c.type].dark : TYPE_COLOR[c.type].light}/>))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>)}
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                  {data.categories.length === 0 ? (<tr><td colSpan={3} className="px-4 py-10 text-center text-[13px] text-gray-400">Bu oy uchun ma&apos;lumot yo&apos;q</td></tr>) : data.categories.map((c, i) => (<tr key={i} className="hover:bg-gray-50/60 dark:hover:bg-white/5 transition-colors duration-100">
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center gap-1.5 text-[11px] font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: (dark ? TYPE_COLOR[c.type].dark : TYPE_COLOR[c.type].light) + "20", color: dark ? TYPE_COLOR[c.type].dark : TYPE_COLOR[c.type].light }}>
