      ...
      <div className="max-w-3xl mx-auto space-y-4">
        ...
        <Card className="rounded-2xl shadow-sm overflow-hidden">
          <div className="flex border-b border-gray-100 dark:border-white/10 px-4">
            <button onClick={() => setTab("grades")} className={cn("flex items-center gap-1.5 px-4 py-3 text-sm font-medium border-b-2 transition-colors", tab === "grades" ? "border-[#5E2CA5] text-[#5E2CA5]" : "border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300")}>
              <GraduationCap className="w-4 h-4"/> Ballar
            </button>
            <button onClick={() => setTab("homework")} className={cn("flex items-center gap-1.5 px-4 py-3 text-sm font-medium border-b-2 transition-colors", tab === "homework" ? "border-[#5E2CA5] text-[#5E2CA5]" : "border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300")}>
              <ClipboardList className="w-4 h-4"/> Uy vazifalari
            </button>
          </div>

          <div className="p-4">
            {tab === "grades" && (<div className="space-y-1.5">
                {data.examScores.map((s) => (<div key={s.id} className="flex items-center justify-between text-sm px-3 py-2.5 rounded-xl bg-gray-50 dark:bg-white/5">
                ...
                {data.exerciseScores.map((s, i) => (<div key={`ex-${i}`} className="flex items-center justify-between text-sm px-3 py-2.5 rounded-xl bg-gray-50 dark:bg-white/5">
                ...
                {data.lessonScores.map((s, i) => (<div key={`ls-${i}`} className="flex items-center justify-between text-sm px-3 py-2.5 rounded-xl bg-gray-50 dark:bg-white/5">
                    <span className="text-gray-600 dark:text-gray-300 truncate">Dars bali · {formatDate(s.date)}</span>
                    <span className="font-medium text-gray-900 dark:text-white flex-shrink-0">{s.score}</span>
                  </div>))}
                {data.examScores.length === 0 && data.exerciseScores.length === 0 && data.lessonScores.length === 0 && (<p className="text-center text-gray-400 py-8">Ballar yo&apos;q</p>)}
              </div>)}

            {tab === "homework" && (<div className="space-y-1.5">
                {data.homework.map((h) => (<div key={h.id} className="flex items-center justify-between text-sm px-3 py-2.5 rounded-xl bg-gray-50 dark:bg-white/5">
                    <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <span className={cn("text-[10px] font-semibold px-2 py-0.5 rounded-full", h.status === "checked" ? "bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-400" : "bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-400")}>
