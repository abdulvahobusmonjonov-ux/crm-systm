...
import { Plus, Filter, Download, Upload, Phone, MessageCircle, Edit, Trash2, LayoutGrid, ChevronLeft, ChevronRight, ChevronDown, Search, X, Users, } from "lucide-react";
...
const BRAND = "#5E2CA5";
const inputCls = "w-full px-3 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-white/10 rounded-xl text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30 focus:border-[#5E2CA5]/50 transition";
export default function LeadsPage() {
    ...
    const assignToGroup = async () => {
        ...
        if (groupMode === "new") {
            if (newGroup.name.trim().length < 1) {
                toast.error("Guruh nomini kiriting");
                setAssigning(false);
                return;
            }
            const res = await apiFetch("/api/groups", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...newGroup, days: groupDays.join(","), courseId: newGroup.courseId || null, teacherId: newGroup.teacherId || null, startDate: newGroup.startDate || null }) });
            if (!res.ok) {
                toast.error("Guruh yaratilmadi");
                setAssigning(false);
                return;
            }
        ...
        await Promise.all([...selected].map(id => apiFetch(`/api/leads/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ groupId }) })));
    ...
    const toggleGroupDay = (d) => setGroupDays(arr => arr.includes(d) ? arr.filter(x => x !== d) : [...arr, d]);
    ...
    const handleBulkStageChange = async (stageId) => {
        if (!selected.size)
            return;
        await Promise.all([...selected].map(id => apiFetch(`/api/leads/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ stageId }) })));
    ...
    const exportExcel = async () => {
        ...
        Object.entries(filters).forEach(([k, v]) => { if (v)
            params.set(k, String(v)); });
        const res = await apiFetch(`/api/leads/export?${params}`);
        if (res.ok) {
    ...
    const handleImport = async () => {
        ...
        if (data.created > 0) {
            toast.success(`${data.created} ta lid qo'shildi`);
            loadLeads();
        }
        if (data.failed > 0)
            toast.error(`${data.failed} ta qatorda xatolik topildi`);
    };
    const chipCls = (active) => cn("px-3.5 py-1.5 rounded-full text-[13px] font-medium transition-colors duration-150 whitespace-nowrap", active
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="relative">
            ...
            <input value={inputSearch} onChange={e => setInputSearch(e.target.value)} placeholder="Ism yoki telefon..." className="pl-9 pr-4 py-2 text-[13px] bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 rounded-xl w-52 focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30 focus:border-[#5E2CA5]/50 text-gray-900 dark:text-white placeholder-gray-400 transition"/>
          </div>
          <button onClick={() => setShowFilters(!showFilters)} className={cn("flex items-center gap-1.5 px-3 py-2 rounded-xl text-[13px] font-medium border transition-colors duration-150", showFilters
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {stages.length > 0 && (<div className="relative">
            <select value={filters.stageId} onChange={e => setFilter("stageId", e.target.value)} className={cn("pl-3.5 pr-8 py-1.5 rounded-full text-[13px] font-medium appearance-none cursor-pointer transition-colors duration-150", filters.stageId
                ? "bg-[#5E2CA5] text-white"
                : "bg-white dark:bg-gray-900 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/5")}>
      ...
      {/* Advanced filters */}
      {showFilters && (<div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-4">
          ...
          {(filters.courseId || filters.source || filters.assignedToId || filters.dateFrom || filters.dateTo) && (<button onClick={() => setFilters(f => ({ ...f, courseId: "", source: "", assignedToId: "", dateFrom: "", dateTo: "", page: 1 }))} className="mt-3 flex items-center gap-1 text-[12px] text-gray-400 hover:text-gray-600 transition-colors">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
        ...
        {/* Bulk actions */}
        {selected.size > 0 && (<div className="px-5 py-2.5 bg-[#5E2CA5]/5 dark:bg-[#5E2CA5]/10 border-b border-[#5E2CA5]/10 flex items-center gap-3 flex-wrap">
            <span className="text-[13px] font-medium text-[#5E2CA5]">{selected.size} ta tanlandi</span>
            <select onChange={e => { if (e.target.value)
            handleBulkStageChange(e.target.value); e.target.value = ""; }} className="px-2.5 py-1 text-xs border border-gray-200 dark:border-white/10 rounded-lg bg-white dark:bg-gray-800 text-gray-700 dark:text-white focus:outline-none">
        ...
        {/* Table */}
        {loading ? (<div className="py-16 text-center text-gray-400 text-sm animate-pulse">Yuklanmoqda...</div>) : leads.length === 0 ? (<EmptyState icon={Users} title="Hech qanday lid topilmadi" description="Yangi lid qo'shib mijozlar bazasini to'ldirishni boshlang." action={<Link href="/leads/new">
              ...
              <thead>
                <tr className="border-b border-gray-100 dark:border-white/5">
                  <th className="w-10 px-4 py-3">
                    <input type="checkbox" checked={selected.size === leads.length && leads.length > 0} onChange={toggleAll} className="h-4 w-4 rounded border-gray-300 accent-[#5E2CA5]"/>
                  </th>
                  {TABLE_HEADERS.map((h, i) => (<th key={i} className="px-3 py-3 text-left text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide whitespace-nowrap">
                      {h}
                    </th>))}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                {leads.map(lead => (<tr key={lead.id} className={cn("hover:bg-gray-50 dark:hover:bg-white/5 transition-colors duration-100", selected.has(lead.id) && "bg-[#5E2CA5]/5 dark:bg-[#5E2CA5]/10")}>
                    <td className="px-4 py-3">
                      <input type="checkbox" checked={selected.has(lead.id)} onChange={() => toggleSelect(lead.id)} className="h-4 w-4 rounded border-gray-300 accent-[#5E2CA5]"/>
                    </td>
                    ...
                    <td className="px-3 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <Link href={`/leads/${lead.id}`} className="text-[13px] font-medium text-gray-900 dark:text-white hover:text-[#5E2CA5] transition-colors duration-150">
                          {lead.fullName}
                        </Link>
                      </div>
                    </td>
                    ...
                    {/* Course */}
                    <td className="px-3 py-3">
                      {lead.course ? (<span className="inline-flex items-center gap-1.5 text-[11px] font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: (lead.course.color || BRAND) + "20", color: lead.course.color || BRAND }}>
                    ...
                    {/* Stage badge */}
                    <td className="px-3 py-3">
                      {lead.stage ? (<span className="inline-flex items-center text-[11px] font-semibold px-2.5 py-1 rounded-full text-white whitespace-nowrap" style={{ backgroundColor: lead.stage.color }}>
                          {lead.stage.name}
                        </span>) : <span className="text-xs text-gray-300">—</span>}
                    </td>
        ...
        {/* Pagination */}
        {pages > 1 && (<div className="px-5 py-3 border-t border-gray-50 dark:border-white/5 flex items-center justify-between">
            <span className="text-[13px] text-gray-400">
              {total} ta natijadan {((filters.page - 1) * 50) + 1}–{Math.min(filters.page * 50, total)} ko&apos;rsatilmoqda
            </span>
            <div className="flex items-center gap-1.5">
              <button onClick={() => setFilters(f => ({ ...f, page: f.page - 1 }))} disabled={filters.page <= 1} className="p-2 rounded-xl border border-gray-200 dark:border-white/10 disabled:opacity-30 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                <ChevronLeft className="w-4 h-4 text-gray-500"/>
              </button>
              <span className="text-[13px] text-gray-500 px-1">{filters.page} / {pages}</span>
              <button onClick={() => setFilters(f => ({ ...f, page: f.page + 1 }))} disabled={filters.page >= pages} className="p-2 rounded-xl border border-gray-200 dark:border-white/10 disabled:opacity-30 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
      ...
      {showGroupModal && (<Modal title="Guruhga qo'shish" subtitle={`${selected.size} ta lid tanlandi`} maxWidth="max-w-lg" onClose={() => setShowGroupModal(false)} footer={<>
            ...
            <div className="flex gap-1 bg-gray-100 dark:bg-gray-800 p-1 rounded-xl mb-5">
              {["new", "existing"].map(mode => (<button key={mode} onClick={() => setGroupMode(mode)} className={cn("flex-1 px-3 py-1.5 text-[13px] font-medium rounded-lg transition-colors", groupMode === mode ? "bg-white dark:bg-gray-900 shadow-sm text-gray-900 dark:text-white" : "text-gray-500")}>
                  {mode === "new" ? "Yangi guruh" : "Mavjud guruh"}
                </button>))}
            </div>

            {groupMode === "existing" ? (<select value={selectedGroupId} onChange={e => setSelectedGroupId(e.target.value)} className={inputCls}>
                ...
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {WEEKDAYS.map(d => (<button key={d.value} type="button" onClick={() => toggleGroupDay(d.value)} className={cn("px-3 py-1 text-[13px] font-medium rounded-lg transition-colors", groupDays.includes(d.value) ? "bg-[#5E2CA5] text-white" : "border border-gray-200 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/5")}>
                        {d.label}
                      </button>))}
                  </div>
                </div>
      ...
      {/* Excel import modal */}
      {showImportModal && (<Modal title="Lidlarni Excel'dan import qilish" subtitle="F.I.Sh va Telefon ustunlari majburiy" maxWidth="max-w-lg" onClose={closeImportModal} footer={<>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="file" accept=".xlsx,.xls" onChange={(e) => { setImportFile(e.target.files?.[0] || null); setImportResult(null); }} className="w-full text-[13px] text-gray-600 dark:text-gray-300 file:mr-3 file:px-3 file:py-1.5 file:rounded-lg file:border-0 file:bg-[#5E2CA5]/10 file:text-[#5E2CA5] file:text-[13px] file:font-medium file:cursor-pointer cursor-pointer"/>
            </div>
            <div>
              ...
              <select value={importSource} onChange={(e) => setImportSource(e.target.value)} className={inputCls}>
                {Object.entries(LEAD_SOURCE_LABELS).filter(([v]) => v !== "WEBSITE").map(([v, l]) => (<option key={v} value={v}>{l}</option>))}
              </select>
            </div>

            {importResult && (<div className="rounded-xl bg-gray-50 dark:bg-white/5 p-3 space-y-2">
                ...
                {importResult.errors.length > 0 && (<div className="max-h-40 overflow-y-auto space-y-1 pr-1">
                    {importResult.errors.map((e, i) => (<p key={i} className="text-[12px] text-red-500 dark:text-red-400">
