...
export default function StudentsPage() {
    ...
    useEffect(() => {
        apiFetch("/api/courses").then(r => r.json()).then(d => setCourses(Array.isArray(d) ? d : []));
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select value={courseId} onChange={e => { setCourseId(e.target.value); setGroupId(""); }} className={selectCls}>
      ...
      {/* Table card */}
      {loading ? (<div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm py-16 text-center text-[13px] text-gray-400 animate-pulse">
          Yuklanmoqda...
        </div>) : students.length === 0 ? (<EmptyState icon={GraduationCap} title="O'quvchi topilmadi" description={search || courseId || groupId ? "Qidiruv yoki filtrlarni o'zgartirib ko'ring" : "Hali ro'yxatga olingan o'quvchi yo'q"}/>) : (<div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                {students.map(s => (<tr key={s.id} onClick={() => router.push(`/students/${s.id}`)} className="hover:bg-gray-50/60 dark:hover:bg-white/5 transition-colors duration-100 cursor-pointer">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <Link href={`/students/${s.id}`} onClick={e => e.stopPropagation()} className="text-[13px] font-medium text-gray-900 dark:text-white hover:text-[#5E2CA5] transition-colors truncate">
                          {s.fullName}
                        </Link>
                      </div>
                    </td>
                    ...
                    {/* Kurs */}
                    <td className="px-4 py-3">
                      {s.course ? (<span className="inline-flex items-center gap-1.5 text-[11px] font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: (s.course.color || BRAND) + "18", color: s.course.color || BRAND }}>
          ...
          {pages > 1 && (<div className="px-5 py-3 border-t border-gray-50 dark:border-white/5 flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5">
                <button onClick={() => setPage(p => p - 1)} disabled={page <= 1} className="p-2 rounded-xl border border-gray-200 dark:border-white/10 disabled:opacity-30 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                  <ChevronLeft className="w-4 h-4 text-gray-500"/>
                </button>
                <span className="text-[13px] text-gray-500 px-1">{page} / {pages}</span>
                <button onClick={() => setPage(p => p + 1)} disabled={page >= pages} className="p-2 rounded-xl border border-gray-200 dark:border-white/10 disabled:opacity-30 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
