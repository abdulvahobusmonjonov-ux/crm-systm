...
export default function PayrollPage() {
    ...
    const [data, setData] = useState({ byUser: [], records: [], total: 0 });
    ...
    const save = async () => {
        ...
        setSaving(true);
        const res = await apiFetch("/api/payroll", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, month }) });
        setSaving(false);
        if (res.ok) {
            toast.success("Saqlandi");
            setShowAdd(false);
            setForm({ userId: "", amount: "", type: "salary", note: "" });
            load();
        }
        else
    ...
    const del = async (id) => {
        if (!(await confirm({ title: "Yozuvni o'chirish", message: "Bu ish haqi yozuvini o'chirishni tasdiqlaysizmi?" })))
            return;
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (<div className="py-16 text-center text-[13px] text-gray-400 animate-pulse">Yuklanmoqda...</div>) : (data.byUser || []).length === 0 ? (<div className="py-16 text-center">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                ...
                return (<tr key={u.userId} className="hover:bg-gray-50/60 dark:hover:bg-white/5 transition-colors duration-100">
                      ...
                      <td className="px-4 py-3 text-[13px] text-gray-500 dark:text-gray-400 whitespace-nowrap">
                        {ROLE_LABELS[u.role] || <span className="text-gray-300">—</span>}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {paid > 0 ? (<span className="inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full bg-green-500/10 text-green-600 whitespace-nowrap">
                            <span className="w-1.5 h-1.5 rounded-full bg-green-500"/> To&apos;langan
                          </span>) : (<span className="inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full bg-gray-100 text-gray-400 dark:bg-white/5 whitespace-nowrap">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                  {data.records.map((r) => (<tr key={r.id} className="hover:bg-gray-50/60 dark:hover:bg-white/5 transition-colors duration-100">
                      <td className="px-4 py-2.5 text-[13px] text-gray-900 dark:text-white whitespace-nowrap">{r.user?.fullName || "—"}</td>
                      <td className="px-4 py-2.5">
                        {r.type === "bonus" ? (<span className="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-amber-500/10 text-amber-600">Bonus</span>) : (<span className="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-gray-100 text-gray-500 dark:bg-white/5">Oylik</span>)}
                      </td>
      ...
      {/* Add modal */}
      {showAdd && (<Modal title={`Ish haqi qo'shish (${month})`} onClose={() => setShowAdd(false)} footer={<>
