...
export default function IncomesPage() {
    ...
    const load = useCallback(async () => {
        ...
        const res = await apiFetch(`/api/incomes?${params}`);
        if (res.ok) {
    ...
    const del = async (id) => {
        if (!(await confirm({ title: "Daromadni o'chirish", message: "Bu daromadni o'chirishni tasdiqlaysizmi?" })))
            return;
        const res = await apiFetch(`/api/incomes/${id}`, { method: "DELETE" });
        if (res.ok) {
    ...
    // Derived — top source
    const srcTotals = incomes.reduce((acc, e) => {
        if (e.source)
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (<div className="py-16 text-center text-[13px] text-gray-400 animate-pulse">Yuklanmoqda...</div>) : incomes.length === 0 ? (<div className="py-16 text-center">
      ...
      {/* Add modal */}
      {showAdd && (<Modal title="Daromad qo'shish" onClose={() => setShowAdd(false)} footer={<>
            ...
            <Input label="Summa (so'm)" type="number" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))} placeholder="1000000"/>

            <Input label="Manba" value={form.source} onChange={e => setForm(f => ({ ...f, source: e.target.value }))} placeholder="Sotuv / Tadbir / Boshqa..."/>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <DatePicker value={form.receivedAt} onChange={e => setForm(f => ({ ...f, receivedAt: e.target.value }))} className={fieldCls}/>
              </div>
            </div>
