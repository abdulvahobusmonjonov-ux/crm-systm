...
import { Plus, Save, Users2, Wallet, Star, Phone, BookOpen, GraduationCap, } from "lucide-react";
...
export default function TeachersPage() {
    ...
    const save = async (t) => {
        ...
        if (res.ok) {
            ...
            toast.error("Saqlab bo'lmadi (faqat admin)");
    };
    const createTeacher = async () => {
        ...
        if (!newForm.username.trim()) {
            toast.error("Username kiriting");
            return;
        }
        if (!newForm.password.trim()) {
        ...
        if (res.ok) {
            ...
            if (typeof err.error === "string") {
                ...
                const firstField = Object.values(err.error.fieldErrors).find((v) => Array.isArray(v) && v.length);
                if (firstField?.[0])
                    msg = firstField[0];
            }
            toast.error(msg);
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {loading ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1, 2, 3, 4, 5, 6].map(i => (<div key={i} className="bg-white dark:bg-gray-900 rounded-2xl h-64 animate-pulse"/>))}
        </div>) : teachers.length === 0 ? (<div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm py-20 text-center">
          ...
          {teachers.map((t) => {
                ...
                return (<div key={t.id} className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col overflow-hidden">
                {/* Top section */}
                <div className="p-5 pb-4">
                  <div className="flex items-start gap-3">
                    {/* Avatar */}
                    <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-white text-[15px] font-bold flex-shrink-0" style={{ backgroundColor: avatarColor }}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: subColor }}/>
                ...
                <div className="px-5 py-3.5 space-y-2.5">
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input value={currentSubject} onChange={e => setEdit(t.id, "subject", e.target.value, t)} placeholder="—" className="w-full px-2.5 py-1.5 text-[12px] border border-gray-200 dark:border-white/10 rounded-lg bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30 transition"/>
                    </div>
                    <div>
                      <label className={labelCls}>Maosh (so&apos;m)</label>
                      <input type="number" value={currentSalary} onChange={e => setEdit(t.id, "salary", e.target.value, t)} placeholder="0" className="w-full px-2.5 py-1.5 text-[12px] border border-gray-200 dark:border-white/10 rounded-lg bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30 transition"/>
                    </div>
                  </div>

                  {/* Phone row + Save button */}
                  <div className="flex items-center justify-between gap-2">
                    {t.phone ? (<a href={`tel:${t.phone}`} className="flex items-center gap-1.5 text-[12px] text-gray-500 dark:text-gray-400 hover:text-[#5E2CA5] transition-colors">
                        <Phone className="w-3.5 h-3.5"/>
                        {formatPhone(t.phone)}
                      </a>) : (<span className="flex items-center gap-1.5 text-[12px] text-gray-300 dark:text-gray-600">
                        <Phone className="w-3.5 h-3.5"/>
                        Telefon yo&apos;q
                      </span>)}
                    <button onClick={() => save(t)} className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-medium transition-colors", isDirty
      ...
      {showCreate && (<Modal title="Yangi o'qituvchi" maxWidth="max-w-sm" onClose={() => setShowCreate(false)} footer={<>
            ...
            <div>
              ...
              <input value={newForm.fullName} onChange={e => setNewForm(f => ({ ...f, fullName: e.target.value }))} placeholder="Ism Familiya" className={fieldCls}/>
            </div>
            <div>
              <label className={labelCls}>Username *</label>
              <input value={newForm.username} onChange={e => setNewForm(f => ({ ...f, username: e.target.value }))} placeholder="teacher1" className={fieldCls}/>
            </div>
            <div>
              <label className={labelCls}>Parol *</label>
              <input type="password" value={newForm.password} onChange={e => setNewForm(f => ({ ...f, password: e.target.value }))} placeholder="••••••••" className={fieldCls}/>
            </div>
