    ...
    return (<>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-6">
          ...
          <ResponsiveContainer width="100%" height={190}>
            <BarChart data={data.sourceDistribution} layout="vertical" margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
              ...
              <YAxis dataKey="source" type="category" tick={{ fontSize: 10, fill: "#9ca3af" }} width={80} axisLine={false} tickLine={false} tickFormatter={v => LEAD_SOURCE_LABELS[v] ?? v}/>
              <Tooltip contentStyle={{ borderRadius: 12, border: "none", boxShadow: "0 4px 24px rgba(0,0,0,0.08)", fontSize: 12 }} cursor={{ fill: "#f5f3ff" }} formatter={v => [v, "Lidlar"]}/>
          ...
          <ResponsiveContainer width="100%" height={160}>
            <PieChart>
              <Pie data={activeStages} dataKey="count" nameKey="name" cx="50%" cy="50%" innerRadius={40} outerRadius={70} paddingAngle={2}>
                {activeStages.map(entry => (<Cell key={entry.stageId} fill={entry.color || "#9ca3af"}/>))}
              </Pie>
              <Tooltip contentStyle={{ borderRadius: 12, border: "none", boxShadow: "0 4px 24px rgba(0,0,0,0.08)", fontSize: 12 }} formatter={(v, n) => [v, n]}/>
            </PieChart>
          </ResponsiveContainer>
          <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1.5">
            {activeStages.slice(0, 4).map(s => (<div key={s.stageId} className="flex items-center gap-1.5">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-6">
          ...
          <ResponsiveContainer width="100%" height={190}>
            <BarChart data={data.topCourses.map(c => ({ name: c.course?.name ?? "Noma'lum", count: c.count }))} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" vertical={false}/>
              <XAxis dataKey="name" tick={{ fontSize: 9, fill: "#9ca3af" }} axisLine={false} tickLine={false}/>
              <YAxis tick={{ fontSize: 10, fill: "#9ca3af" }} axisLine={false} tickLine={false}/>
              <Tooltip contentStyle={{ borderRadius: 12, border: "none", boxShadow: "0 4px 24px rgba(0,0,0,0.08)", fontSize: 12 }} cursor={{ fill: "#f5f3ff" }}/>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm">
        ...
        <div className="divide-y divide-gray-50 dark:divide-white/5 max-h-72 overflow-y-auto">
          {data.recentActivities.map(act => (<div key={act.id} className="flex items-start gap-3 px-6 py-3">
