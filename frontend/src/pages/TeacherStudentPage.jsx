...
export default function TeacherStudentDetailPage() {
    ...
    const load = useCallback(async () => {
        setLoading(true);
        const leadRes = await apiFetch(`/api/teacher/students/${id}`);
        if (!leadRes.ok) {
        ...
        setStudent(leadData);
        if (showPayments) {
            const paymentsRes = await apiFetch(`/api/teacher/students/${id}/payments`);
            if (paymentsRes.ok) {
    ...
    const tabs = [
        ...(showPayments ? [{ key: "payments", label: "To'lovlar", icon: Wallet }] : []),
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6">
      <div className="max-w-4xl mx-auto space-y-4" style={{ fontFamily: "'Inter', sans-serif" }}>
          ...
          <CardContent className="p-5 sm:p-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {showContacts && student.phone && (<div className="flex items-center gap-4 mt-2 flex-wrap text-sm">
        ...
        <div className={`grid grid-cols-1 gap-4 ${showPayments ? "sm:grid-cols-3" : "sm:grid-cols-2"}`}>
          {showPayments && (<div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-5 flex items-center gap-4">
