...
export default function NewLeadPage() {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            const res = await apiFetch(`/api/leads?search=${encodeURIComponent(phone)}&limit=1`);
            if (res.ok) {
                const data = await res.json();
                const match = data.leads.find((l) => l.phone === phone || l.phone.replace(/\D/g, "") === phone.replace(/\D/g, ""));
    ...
    const createCourse = async () => {
        if (newCourseName.trim().length < 2) {
            toast.error("Kurs nomi kamida 2 harf");
            return;
        }
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6" style={{ fontFamily: "'Inter', sans-serif" }}>
      <div className="max-w-3xl mx-auto space-y-5">
        ...
        <div className="flex items-center gap-3">
          <Link href="/leads" className="p-2 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 text-gray-500 hover:text-[#5E2CA5] hover:border-[#5E2CA5]/30 transition-colors shadow-sm">
        ...
        {/* Duplicate warning */}
        {duplicate && (<div className="flex items-center gap-3 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div className={sectionCls}>
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => setShowNewCourse(s => !s)} className="px-3 py-2 text-[13px] font-medium rounded-xl border border-[#5E2CA5]/30 text-[#5E2CA5] hover:bg-[#5E2CA5]/8 transition whitespace-nowrap" style={{ backgroundColor: showNewCourse ? "#5E2CA508" : undefined }}>
                  + Yangi kurs
                </button>
              </div>
              {showNewCourse && (<div className="mt-2 flex gap-2">
                  <input value={newCourseName} onChange={e => setNewCourseName(e.target.value)} placeholder="Kurs nomi (masalan: IT Foundation)" className={cn(fieldCls, "flex-1")}/>
                  <button type="button" onClick={createCourse} disabled={creatingCourse} className="px-4 py-2 rounded-xl text-[13px] font-medium bg-[#5E2CA5] hover:bg-[#4a2280] text-white transition disabled:opacity-60">
            ...
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {WEEKDAYS.map(d => (<button key={d.value} type="button" onClick={() => toggleDay(d.value)} className={cn("px-3 py-1.5 text-[12px] font-medium rounded-lg transition-colors", selectedDays.includes(d.value)
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <input type="time" value={timeFrom} onChange={e => setTimeFrom(e.target.value)} className="px-3 py-2 text-[13px] border border-gray-200 dark:border-white/10 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30 focus:border-[#5E2CA5]/50 transition"/>
                <span className="text-gray-400">—</span>
                <input type="time" value={timeTo} onChange={e => setTimeTo(e.target.value)} className="px-3 py-2 text-[13px] border border-gray-200 dark:border-white/10 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30 focus:border-[#5E2CA5]/50 transition"/>
                {(timeFrom || timeTo) && (<span className="text-[13px] font-medium text-[#5E2CA5]">{timeFrom || "—"}–{timeTo || "—"}</span>)}
              </div>
            </div>
          </div>
          ...
          <div className={sectionCls}>
            ...
            <textarea {...register("notes")} rows={4} placeholder="Mijoz haqida izoh, suhbat tafsilotlari..." className={fieldCls}/>
          ...
          <div className="flex justify-end gap-3 pb-6">
            <Link href="/leads">
              <button type="button" className="px-5 py-2.5 rounded-xl text-[13px] font-medium border border-gray-200 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors bg-white dark:bg-gray-900">
                Bekor qilish
              </button>
            </Link>
            <button type="submit" disabled={saving} className="px-6 py-2.5 rounded-xl text-[13px] font-semibold bg-[#5E2CA5] hover:bg-[#4a2280] text-white shadow-sm transition-colors disabled:opacity-60">
