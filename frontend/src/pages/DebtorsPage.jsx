...
export default function DebtorsPage() {
    ...
    const sendReminder = async (debtor) => {
        ...
        const res = await apiFetch(`/api/leads/${debtor.id}/notify`, {
            method: "POST",
        ...
        setSendingId(null);
        if (res.ok)
            toast.success(`${debtor.fullName} ga eslatma yuborildi`);
        else {
            const err = await res.json().catch(() => ({}));
            toast.error(err.error || "Yuborilmadi");
        }
    };
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (<div className="py-16 text-center text-[13px] text-gray-400 animate-pulse">Yuklanmoqda...</div>) : debtors.length === 0 ? (<div className="py-16 text-center">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                {debtors.map(d => (<tr key={d.id} className="hover:bg-gray-50/60 dark:hover:bg-white/5 transition-colors duration-100">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          ...
                          <a href={phoneToTel(d.phone)} className="text-[11px] text-gray-400 font-mono hover:text-[#5E2CA5] transition-colors">
                            {formatPhone(d.phone)}
                          </a>
                        </div>
                      </div>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-1">
                        ...
                        {d.course && (<span className="inline-flex items-center gap-1.5 text-[11px] font-medium px-2 py-0.5 rounded-full w-fit" style={{
                        backgroundColor: (d.courseColor || BRAND) + "18",
                        color: d.courseColor || BRAND,
                    }}>
                            <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: d.courseColor || BRAND }}/>
                            {d.course}
                          </span>)}
                    ...
                    <td className="px-4 py-3 text-right">
                      ...
                      {d.paid > 0 && (<p className="text-[11px] text-gray-400 mt-0.5 whitespace-nowrap">
                    ...
                    {/* Eslatma */}
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => sendReminder(d)} disabled={sendingId === d.id} className={cn("inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[12px] font-semibold transition-colors whitespace-nowrap", "bg-[#5E2CA5] hover:bg-[#4a2280] text-white disabled:opacity-60")}>
