...
const CYCLE = { present: "absent", absent: "excused", excused: null };
...
export default function AttendanceTab({ groupId, month }) {
    ...
    const load = useCallback(async () => {
        ...
        setError(false);
        try {
            const res = await apiFetch(`/api/teacher/groups/${groupId}/attendance?month=${month}`);
            if (!res.ok) {
                setError(true);
                setLoading(false);
                return;
    ...
    const save = async () => {
        ...
        const results = await Promise.all(activeDates.map(async (date) => {
            ...
            if (!records.length)
                return true;
            try {
                const res = await apiFetch(`/api/teacher/groups/${groupId}/attendance`, {
    ...
    return (<div className="space-y-3">
      <div className="flex items-center gap-4 flex-wrap">
        ...
        {students.length > 0 && canEdit && (<button onClick={save} disabled={saving} className="ml-auto flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-semibold bg-[#5E2CA5] hover:bg-[#4a2280] text-white shadow-sm transition-colors disabled:opacity-60">
                ...
                return (<tr key={s.leadId} className={cn("hover:bg-gray-50/80 dark:hover:bg-white/5 transition-colors duration-100", rowIdx % 2 === 1 && "bg-gray-50/30 dark:bg-white/[0.02]")}>
                      ...
                      {days.map((d) => {
                        ...
                        return (<td key={d} className={cn("px-0 py-2 text-center", isToday && "bg-[#5E2CA5]/5 dark:bg-[#5E2CA5]/8")}>
                            <button onClick={() => toggleCell(d, s.leadId)} disabled={!canEdit} title={!canEdit ? "Davomat belgilash uchun ruxsat yo'q" : status === "present" ? "Keldi" : status === "absent" ? "Kelmadi" : status === "excused" ? "Sababli" : weekend ? "Dam olish kuni" : "Belgilang"} className={cn("w-6 h-6 rounded-full mx-auto flex items-center justify-center text-[9px] font-bold transition-all duration-100", !canEdit && "cursor-not-allowed opacity-80", status === "present" ? "bg-emerald-500 text-white hover:bg-emerald-600 shadow-sm"
