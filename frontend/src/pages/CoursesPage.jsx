...
export default function CoursesPage() {
    ...
    const toggleActive = async (c) => {
        await apiFetch(`/api/courses/${c.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ isActive: !c.isActive }) });
        load();
    };
    const { confirm, dialog } = useConfirm();
    const deleteCourse = async (c) => {
        if (!(await confirm({ title: "Kursni o'chirish", message: `"${c.name}" kursini o'chirishni tasdiqlaysizmi?` })))
            return;
        const res = await apiFetch(`/api/courses/${c.id}`, { method: "DELETE" });
        if (res.ok) {
            toast.success("Kurs o'chirildi");
            load();
        }
        else
            toast.error("O'chirib bo'lmadi — bog'liq lidlar mavjud bo'lishi mumkin");
    };
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <button onClick={openCreate} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-medium bg-[#5E2CA5] hover:bg-[#4a2280] text-white shadow-sm transition-colors">
      ...
      {loading ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        </div>) : courses.length === 0 ? (<div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm py-20 text-center">
          <GraduationCap className="w-10 h-10 text-gray-300 mx-auto mb-3"/>
          <p className="text-gray-400 font-medium text-[13px]">Hali kurs yo&apos;q</p>
          <button onClick={openCreate} className="mt-4 inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-medium bg-[#5E2CA5] hover:bg-[#4a2280] text-white shadow-sm transition-colors">
            <Plus className="w-4 h-4"/> Birinchi kursni qo&apos;shing
          </button>
        </div>) : (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {courses.map(c => (<div key={c.id} className={`bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-5 ${!c.isActive ? "opacity-60" : ""}`}>
              {/* Top: icon, name, status badge */}
              <div className="flex items-start justify-between gap-2 mb-3">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-11 h-11 rounded-full flex items-center justify-center flex-shrink-0 text-white font-bold text-[15px]" style={{ backgroundColor: c.color || "#5E2CA5" }}>
      ...
      {/* Modal */}
      {modalOpen && (<Modal title={editing ? "Kursni tahrirlash" : "Yangi kurs"} onClose={() => setModalOpen(false)}>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {COLORS.map(color => (<button key={color} type="button" onClick={() => setValue("color", color)} className={`w-7 h-7 rounded-lg transition ${selectedColor === color ? "ring-2 ring-offset-2 ring-[#5E2CA5] scale-110" : ""}`} style={{ backgroundColor: color }}/>))}
              </div>
            </div>
