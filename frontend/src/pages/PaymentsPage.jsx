...
import { Plus, Trash2, X, Search, Wallet, CalendarDays, AlertCircle, Receipt, Download, } from "lucide-react";
...
const inputCls = "w-full px-3 py-2 text-[13px] bg-white dark:bg-gray-800 border border-gray-200 dark:border-white/10 rounded-xl text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30 focus:border-[#5E2CA5]/50 transition";
...
export default function PaymentsPage() {
    ...
    const [summary, setSummary] = useState({ total: 0, count: 0, monthTotal: 0, month: "" });
    ...
    const [presetLead, setPresetLead] = useState(null);
    const [view, setView] = useState("payments");
    const [debtors, setDebtors] = useState({ debtors: [], totalDebt: 0, count: 0, month: "" });
    ...
    const setFilterQ = (v) => { setQ(v); setPage(1); };
    const setFilterMonth = (v) => { setMonth(v); setPage(1); };
    const loadDebtors = useCallback(async () => {
        const params = new URLSearchParams({ month: debtMonth });
        const res = await apiFetch(`/api/debtors?${params.toString()}`);
        setDebtors(await res.json());
    }, [debtMonth]);
    useEffect(() => { loadDebtors(); }, [loadDebtors]);
    const { confirm, dialog } = useConfirm();
    const del = async (id) => {
        if (!(await confirm({ title: "To'lovni o'chirish", message: "Bu to'lovni o'chirishni tasdiqlaysizmi?" })))
            return;
        ...
        if (res.ok) {
            ...
            toast.error("O'chirib bo'lmadi (faqat admin)");
    };
    const exportPayments = async () => {
        ...
        if (!all.length) {
            toast.error("Eksport qilish uchun to'lov yo'q");
            return;
        }
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={exportPayments} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-medium border border-gray-200 dark:border-white/10 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {kpiCards.map(card => {
            ...
            return (<div key={card.label} className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-5 flex items-start gap-4">
      ...
      <div className="flex gap-1 bg-white dark:bg-gray-900 p-1 rounded-xl w-fit shadow-sm border border-gray-100 dark:border-white/5">
        {["payments", "debtors"].map(v => (<button key={v} onClick={() => setView(v)} className={cn("px-4 py-2 text-[13px] font-medium rounded-lg transition-colors duration-150", view === v
      ...
      {view === "payments" && (<>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input value={q} onChange={e => setFilterQ(e.target.value)} placeholder="O'quvchi ismi yoki telefon..." className="w-full pl-9 pr-3 py-2 text-[13px] border border-gray-200 dark:border-white/10 rounded-xl bg-white dark:bg-gray-900 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30 focus:border-[#5E2CA5]/50 transition"/>
            </div>
            <MonthPicker value={month} onChange={e => setFilterMonth(e.target.value)} className="px-3 py-2 text-[13px]"/>
            {month && (<button onClick={() => setFilterMonth("")} className="px-3 py-2 text-[13px] font-medium text-gray-500 border border-gray-200 dark:border-white/10 rounded-xl hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                Tozalash
              </button>)}
          </div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                  {loading ? (<tr><td colSpan={8} className="px-4 py-12 text-center text-[13px] text-gray-400 animate-pulse">Yuklanmoqda...</td></tr>) : payments.length === 0 ? (<tr>
                      <td colSpan={8} className="p-0">
                        <EmptyState icon={Receipt} title="To'lov topilmadi" description="Hozircha hech qanday to'lov qayd etilmagan." className="shadow-none rounded-none"/>
                      </td>
                    </tr>) : payments.map(p => (<tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors duration-100">
                      {/* O'quvchi + avatar */}
                      ...
                      {/* Guruh/Kurs */}
                      <td className="px-4 py-3">
                        {p.course?.name || p.group?.name ? (<span className="inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full bg-[#5E2CA5]/8 text-[#5E2CA5]" style={{ backgroundColor: "#5E2CA510" }}>
                            {p.course?.name || p.group?.name}
                          </span>) : <span className="text-[13px] text-gray-300">—</span>}
                      </td>
                    ...
                    : <span className="text-[13px] text-gray-300">—</span>}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          <a href={`/receipt/${p.id}`} target="_blank" rel="noreferrer" className="px-2 py-1 text-[11px] font-medium rounded-lg border border-gray-200 dark:border-white/10 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                            Chek
                          </a>
                          <button onClick={() => del(p.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors">
            ...
            {pages > 1 && (<div className="px-5 py-3 border-t border-gray-50 dark:border-white/5 flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  <button onClick={() => setPage(p => p - 1)} disabled={page <= 1} className="px-3 py-1.5 rounded-xl border border-gray-200 dark:border-white/10 disabled:opacity-30 text-[13px] text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                    Oldingi
                  </button>
                  <span className="text-[13px] text-gray-500 px-1">{page} / {pages}</span>
                  <button onClick={() => setPage(p => p + 1)} disabled={page >= pages} className="px-3 py-1.5 rounded-xl border border-gray-200 dark:border-white/10 disabled:opacity-30 text-[13px] text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                    Keyingi
                  </button>
                </div>
              </div>)}
      ...
      {/* ── DEBTORS view ── */}
      {view === "debtors" && (<div className="space-y-4">
          <div className="flex items-center gap-3 flex-wrap">
            <MonthPicker value={debtMonth} onChange={e => setDebtMonth(e.target.value)} className="px-3 py-2 text-[13px]"/>
            {(debtors.debtors?.length > 0) && (<p className="text-[13px] text-gray-500">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                  {(debtors.debtors || []).length === 0 ? (<tr>
                      <td colSpan={6} className="px-4 py-16 text-center">
                        <p className="text-2xl mb-2">🎉</p>
                        <p className="text-[13px] text-gray-400">Qarzdor yo&apos;q</p>
                      </td>
                    </tr>) : debtors.debtors.map((d) => (<tr key={d.id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors duration-100">
                      {/* O'quvchi + avatar */}
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                      ...
                      <td className="px-4 py-3 text-right">
                        <button onClick={() => { setPresetLead({ id: d.id, fullName: d.fullName, phone: d.phone }); setShowAdd(true); }} className="px-3 py-1.5 text-[12px] font-semibold rounded-xl bg-[#5E2CA5] hover:bg-[#4a2280] text-white transition-colors">
                          To&apos;lov
                        </button>
                      </td>
                    </tr>))}
      ...
      {showAdd && (<AddPaymentModal presetLead={presetLead} onClose={() => { setShowAdd(false); setPresetLead(null); }} onSaved={() => { setShowAdd(false); setPresetLead(null); load(); loadDebtors(); }}/>)}
    </div>);
}
function AddPaymentModal({ presetLead, onClose, onSaved }) {
    const [leadQ, setLeadQ] = useState("");
    const [leadResults, setLeadResults] = useState([]);
    const [lead, setLead] = useState(presetLead || null);
    ...
    useEffect(() => {
        apiFetch("/api/payment-types").then(r => r.json())
            .then(d => setTypes(Array.isArray(d) ? d.filter((t) => t.isActive) : []))
            .catch(() => { });
    }, []);
    useEffect(() => {
        if (lead || leadQ.length < 1) {
            setLeadResults([]);
            return;
        }
        const t = setTimeout(async () => {
            const res = await apiFetch(`/api/leads?search=${encodeURIComponent(leadQ)}&limit=8`);
    ...
    return (<Modal title="To'lov qabul qilish" onClose={onClose} footer={<>
        ...
        <div className="relative">
          ...
          {lead ? (<div className="flex items-center justify-between px-3 py-2 rounded-xl border border-[#5E2CA5]/30 bg-[#5E2CA5]/5 dark:bg-[#5E2CA5]/10">
              <div className="flex items-center gap-2">
              ...
              <input value={leadQ} onChange={e => setLeadQ(e.target.value)} placeholder="Ism yoki telefon bo'yicha qidiring..." className={fieldCls}/>
              {leadResults.length > 0 && (<div className="absolute z-10 mt-1 w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-white/10 rounded-xl shadow-lg max-h-52 overflow-y-auto">
                  {leadResults.map(l => (<button key={l.id} onClick={() => { setLead(l); setLeadResults([]); }} className="w-full text-left px-3 py-2 text-[13px] hover:bg-gray-50 dark:hover:bg-white/5 transition-colors border-b border-gray-50 dark:border-white/5 last:border-0">
