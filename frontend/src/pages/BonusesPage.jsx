...
export default function BonusesPage() {
    ...
    const load = useCallback(async () => {
        ...
        if (res.ok) {
            ...
            setRecords((data.records || []).filter((r) => r.type === "bonus"));
        }
        setLoading(false);
    }, [month]);
    useEffect(() => { load(); }, [load]);
    ...
    const del = async (id) => {
        if (!(await confirm({ title: "Bonusni o'chirish", message: "Bu bonus yozuvini o'chirishni tasdiqlaysizmi?" })))
            return;
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (<div className="py-16 text-center text-[13px] text-gray-400 animate-pulse">Yuklanmoqda...</div>) : records.length === 0 ? (<div className="py-16 text-center">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                {records.map(r => (<tr key={r.id} className="hover:bg-gray-50/60 dark:hover:bg-white/5 transition-colors duration-100">
      ...
      {/* Add modal */}
      {showAdd && (<Modal title={`Bonus qo'shish (${month})`} onClose={() => setShowAdd(false)} footer={<>
          ...
          <div>
            ...
            <input value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} placeholder="Masalan: Yaxshi natija uchun" className={fieldCls}/>
          </div>
        </Modal>)}
    </div>);
}
