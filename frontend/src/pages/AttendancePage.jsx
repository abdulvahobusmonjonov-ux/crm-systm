...
const CYCLE = { present: "absent", absent: null };
const next = (s) => CYCLE[s ?? ""] ?? "present";
function todayStr() {
    const d = new Date();
...
export default function AttendancePage() {
    ...
    useEffect(() => {
        apiFetch("/api/groups?limit=500").then(r => r.json()).then(d => {
            const list = Array.isArray(d?.groups) ? d.groups : [];
            setGroups(list);
            if (list.length && !groupId)
    ...
    const load = useCallback(async () => {
        ...
        const res = await apiFetch(`/api/attendance?groupId=${groupId}&month=${month}`);
        ...
        if (data.journal) {
            ...
            const dayRes = await apiFetch(`/api/attendance?groupId=${groupId}&date=${today}`);
            const dayData = await dayRes.json();
            const dayStudents = dayData.students || [];
            if (!data.students?.length)
                setStudents(dayStudents);
            const j = {};
            dayStudents.forEach(s => { if (s.status) {
                if (!j[today])
                    j[today] = {};
                j[today][s.leadId] = s.status;
            } });
            setJournal(j);
        }
        setLoading(false);
    ...
    const toggleCell = (date, leadId) => {
        setJournal(prev => {
            const dayData = { ...(prev[date] || {}) };
            dayData[leadId] = next(dayData[leadId]);
            if (dayData[leadId] === null)
                delete dayData[leadId];
    ...
    const save = async () => {
        ...
        if (!activeDates.length) {
            toast.error("Hech narsa belgilanmagan");
            return;
        }
        setSaving(true);
        await Promise.all(activeDates.map(date => {
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <MonthPicker value={month} onChange={e => setMonth(e.target.value)} className={selectCls}/>

          <button onClick={() => setAll("present")} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-[13px] font-medium bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
      ...
      <div className="flex items-center gap-4 flex-wrap">
        ...
        {students.length > 0 && (<div className="flex items-center gap-2 ml-auto">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (<div className="py-16 text-center text-[13px] text-gray-400 animate-pulse">Yuklanmoqda...</div>) : students.length === 0 ? (<div className="py-16 text-center">
            ...
            <table className="w-full border-collapse">
              <thead>
                ...
                return (<th key={d} className={cn("px-0 py-3 text-center text-[11px] font-medium min-w-[32px] w-8", weekend ? "text-gray-300 dark:text-gray-600" : "text-gray-400 dark:text-gray-500", isToday && "text-[#5E2CA5] dark:text-purple-400 font-bold")}>
                        {day}
                      </th>);
            })}
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                ...
                return (<tr key={s.leadId} className={cn("hover:bg-gray-50/80 dark:hover:bg-white/5 transition-colors duration-100", rowIdx % 2 === 1 && "bg-gray-50/30 dark:bg-white/[0.02]")}>
                      {/* Student cell — sticky */}
                      ...
                      {days.map(d => {
                        ...
                        const weekend = isWeekend(d);
                        const isToday = d === today;
                        return (<td key={d} className={cn("px-0 py-2 text-center", isToday && "bg-[#5E2CA5]/5 dark:bg-[#5E2CA5]/8")}>
                            <button onClick={() => !weekend && toggleCell(d, s.leadId)} disabled={weekend} title={status === "present" ? "Keldi" :
                                status === "absent" ? "Kelmadi" :
                                    status === "late" ? "Kech keldi" :
                                        status === "excused" ? "Sababli" :
                                            weekend ? "Dam olish kuni" : "Belgilang"} className={cn("w-6 h-6 rounded-full mx-auto flex items-center justify-center text-[8px] font-bold transition-all duration-100", status === "present"
                      ...
                      <td className="px-4 py-2 text-center border-l border-gray-100 dark:border-white/5">
                        {pct !== null ? (<span className={cn("text-[13px] font-bold tabular-nums", pctColor)}>
                            {pct}%
                          </span>) : (<span className="text-[12px] text-gray-300 dark:text-gray-600">—</span>)}
                      </td>
                    </tr>);
            })}
              </tbody>
      ...
      {students.length > 0 && (<div className="flex justify-end">
          <button onClick={save} disabled={saving} className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-[13px] font-semibold bg-[#5E2CA5] hover:bg-[#4a2280] text-white shadow-sm transition-colors disabled:opacity-60">
