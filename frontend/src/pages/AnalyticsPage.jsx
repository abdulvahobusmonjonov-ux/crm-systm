...
export default function AnalyticsPage() {
    ...
    useEffect(() => {
        apiFetch("/api/analytics").then(r => r.json()).then(d => { setData(d); setLoading(false); });
    }, []);
    ...
    if (!data)
        return <div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 flex items-center justify-center text-[13px] text-gray-400">Ma&apos;lumot yo&apos;q</div>;
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between flex-wrap gap-3">
        ...
        <div className="inline-flex items-center gap-1 bg-white dark:bg-gray-900 rounded-xl shadow-sm p-1">
          {PERIODS.map(p => (<button key={p.key} onClick={() => setPeriod(p.key)} className={cn("px-3 py-1.5 rounded-lg text-[13px] font-medium transition-colors", period === p.key ? "bg-[#5E2CA5] text-white shadow-sm" : "text-gray-500 hover:text-gray-700 dark:text-gray-400")}>
              {p.label}
            </button>))}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-5">
        ...
        <div className="flex items-end gap-3 h-48">
          {monthly.map((m) => (<div key={m.month} className="flex-1 flex flex-col items-center gap-2">
              <div className="w-full flex items-end justify-center" style={{ height: "150px" }}>
                <div className="w-full max-w-[48px] rounded-t-lg transition-all" style={{ height: `${(m.revenue / maxRev) * 100}%`, minHeight: m.revenue > 0 ? "4px" : "0", backgroundColor: "#5E2CA5" }} title={money(m.revenue)}/>
              </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                {data.bySource.map((s) => (<tr key={s.source} className="hover:bg-gray-50/60 dark:hover:bg-white/5 transition-colors duration-100">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                {data.byManager.map((m, i) => (<tr key={i} className="hover:bg-gray-50/60 dark:hover:bg-white/5 transition-colors duration-100">
