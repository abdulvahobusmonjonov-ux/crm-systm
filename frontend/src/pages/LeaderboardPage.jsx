    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-5 sm:space-y-6" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-0.5 bg-white dark:bg-gray-900 p-1 rounded-xl shadow-sm border border-gray-100 dark:border-white/5">
          {PERIODS.map(p => (<button key={p.key} onClick={() => setPeriod(p.key)} className={cn("px-4 py-1.5 text-[13px] font-medium rounded-lg transition-colors duration-150", period === p.key
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          <div className="flex items-end justify-center gap-4 h-52">
            {[1, 0, 2].map(i => (<div key={i} className={cn("bg-white dark:bg-gray-900 rounded-2xl animate-pulse w-36", i === 0 ? "h-48" : i === 1 ? "h-36" : "h-28")}/>))}
          </div>
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm h-64 animate-pulse"/>
        </div>) : list.length === 0 ? (<div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm py-20 text-center">
          ...
          {/* ── Podium ── */}
          {top3.length >= 1 && (<div className="flex items-end justify-center gap-3 pt-4 pb-2">
              {PODIUM_ORDER.map((rankIdx, col) => {
                    const student = top3[rankIdx];
                    if (!student)
                        return <div key={col} className="w-36"/>;
                    const medal = MEDAL[rankIdx];
                    const isFirst = rankIdx === 0;
                    return (<div key={col} className="flex flex-col items-center gap-2 select-none">
                    {/* Avatar with medal ring */}
                    <div className="relative">
                      <div className={cn("rounded-full flex items-center justify-center text-white font-bold flex-shrink-0 ring-4", PODIUM_AVATAR[col], isFirst ? "text-xl" : rankIdx === 1 ? "text-base" : "text-sm")} style={{
                      ...
                      {/* Medal badge */}
                      <span className="absolute -bottom-1 -right-1 text-base leading-none" style={{ filter: "drop-shadow(0 1px 2px rgba(0,0,0,0.2))" }}>
                        {medal.label}
                      </span>
                    </div>
                    ...
                    <div className="text-center max-w-[120px]">
                      <p className={cn("font-bold text-gray-900 dark:text-white leading-tight truncate", isFirst ? "text-[14px]" : "text-[12px]")}>
                        {student.fullName.split(" ")[0]}
                      </p>
                      {student.group?.name && (<p className="text-[10px] text-gray-400 truncate">{student.group.name}</p>)}
                    </div>

                    {/* Podium block */}
                    <div className={cn("w-32 rounded-t-2xl flex flex-col items-center justify-center gap-1 shadow-lg", PODIUM_HEIGHT[col])} style={{ backgroundColor: medal.bg, borderTop: `3px solid ${medal.border}` }}>
          ...
          {rest.length > 0 && (<div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
              ...
              <div className="divide-y divide-gray-50 dark:divide-white/5">
                {rest.map((student, idx) => {
                    ...
                    return (<div key={student.id} className="flex items-center gap-4 px-5 py-3 hover:bg-gray-50/60 dark:hover:bg-white/5 transition-colors duration-100">
                      {/* Rank */}
                      ...
                      {/* Avatar */}
                      <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-[11px] font-bold flex-shrink-0" style={{ backgroundColor: color }}>
                        {getInitials(student.fullName)}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {(student.group?.name || student.course?.name) && (<p className="text-[11px] text-gray-400 truncate">
          ...
          {list.length > 0 && rest.length === 0 && list.length <= 3 && (<p className="text-center text-[12px] text-gray-400 pb-2">
