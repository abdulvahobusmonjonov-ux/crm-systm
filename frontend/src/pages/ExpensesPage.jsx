...
const METHODS = { cash: "Naqd", card: "Karta", transfer: "O'tkazma", other: "Boshqa" };
...
export default function ExpensesPage() {
    ...
    const del = async (id) => {
        if (!(await confirm({ title: "Xarajatni o'chirish", message: "Bu xarajatni o'chirishni tasdiqlaysizmi?" })))
            return;
    ...
    const exportExpenses = () => {
        if (!expenses.length) {
            toast.error("Eksport qilish uchun xarajat yo'q");
            return;
        }
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={exportExpenses} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-medium border border-gray-200 dark:border-white/10 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (<div className="py-16 text-center text-[13px] text-gray-400 animate-pulse">Yuklanmoqda...</div>) : expenses.length === 0 ? (<div className="py-16 text-center">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                ...
                return (<tr key={e.id} className="hover:bg-gray-50/60 dark:hover:bg-white/5 transition-colors duration-100">
                      ...
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full whitespace-nowrap" style={{ backgroundColor: color + "18", color }}>
                      ...
                      <td className="px-4 py-3 text-right">
                        <button onClick={() => del(e.id)} className="p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors">
      ...
      {showAdd && (<Modal title="Xarajat qo'shish" onClose={() => setShowAdd(false)} footer={<>
            ...
            <Input label="Kategoriya" value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} placeholder="Ijara / Kommunal / Reklama..."/>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <DatePicker value={form.spentAt} onChange={e => setForm(f => ({ ...f, spentAt: e.target.value }))} className={fieldCls}/>
              </div>
            </div>

            <div>
              <label className={labelCls}>Izoh (ixtiyoriy)</label>
              <input value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} placeholder="..." className={fieldCls}/>
            </div>
