...
function actionBadgeCls(action) {
    if (action.includes("payment_created"))
        return "bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-400";
    if (action.includes("status") || action.includes("stage"))
        return "bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-400";
    if (action.includes("lead_created"))
        return "bg-blue-100 dark:bg-blue-500/15 text-blue-700 dark:text-blue-400";
    if (action.includes("expense") || action.includes("deleted"))
        return "bg-red-100 dark:bg-red-500/15 text-red-700 dark:text-red-400";
    if (action.includes("login"))
        return "bg-gray-100 dark:bg-white/8 text-gray-600 dark:text-gray-400";
...
export default function LogsPage() {
    ...
    useEffect(() => {
        apiFetch("/api/logs").then(r => r.json()).then(d => { setLogs(Array.isArray(d) ? d : []); setLoading(false); });
    }, []);
    const exportCSV = () => {
        ...
        a.href = url;
        a.download = `jurnallar_${new Date().toISOString().slice(0, 10)}.csv`;
        a.click();
    ...
    return (<div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (<div className="py-16 text-center text-[13px] text-gray-400 animate-pulse">Yuklanmoqda...</div>) : logs.length === 0 ? (<div className="py-16 text-center">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                {logs.map(l => (<tr key={l.id} className="hover:bg-gray-50/60 dark:hover:bg-white/5 transition-colors duration-100">
                    ...
                    {/* Amal — rangli badge */}
                    <td className="px-4 py-3">
                      <span className={cn("inline-flex items-center text-[11px] font-semibold px-2.5 py-1 rounded-full whitespace-nowrap", actionBadgeCls(l.action))}>
                        {ACTIONS[l.action] || l.action}
                      </span>
                    </td>
