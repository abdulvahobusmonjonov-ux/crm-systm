...
export default function ScoresTab({ groupId, month }) {
    ...
    const load = useCallback(async () => {
        ...
        setError(false);
        try {
            const res = await apiFetch(`/api/teacher/groups/${groupId}/scores?month=${month}`);
            if (!res.ok) {
                setError(true);
                setLoading(false);
                return;
    ...
    useEffect(() => { load(); }, [load]);
    const setScore = (date, leadId, value) => {
    ...
    const save = async () => {
        ...
        setSaving(true);
        const results = await Promise.all(activeDates.map(async (date) => {
            const records = Object.entries(journal[date] || {})
                .filter(([, score]) => score !== null && score !== undefined)
                .map(([leadId, score]) => ({ leadId, score: score }));
            if (!records.length)
                return true;
            try {
                const res = await apiFetch(`/api/teacher/groups/${groupId}/scores`, {
        ...
        if (results.every(Boolean))
            ...
            toast.error("Ba'zi kunlar saqlanmadi");
    };
    const days = daysInMonth(month);
    const today = todayStr();
    const getAverage = (leadId) => {
        const values = days.map((d) => journal[d]?.[leadId]).filter((v) => typeof v === "number");
        return values.length ? Math.round(values.reduce((a, b) => a + b, 0) / values.length) : null;
    };
    return (<div className="space-y-3">
      <div className="flex items-center justify-end">
        {students.length > 0 && canEdit && (<button onClick={save} disabled={saving} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-semibold bg-[#5E2CA5] hover:bg-[#4a2280] text-white shadow-sm transition-colors disabled:opacity-60">
            <Save className="w-3.5 h-3.5"/> {saving ? "Saqlanmoqda..." : "Saqlash"}
          </button>)}
      </div>

      <Card className="overflow-hidden">
        {loading ? (<div className="py-16 text-center text-[13px] text-gray-400 animate-pulse">Yuklanmoqda...</div>) : error ? (<div className="py-16 text-center text-[13px] text-red-500">Ma&apos;lumotni yuklab bo&apos;lmadi. Sahifani yangilab ko&apos;ring.</div>) : students.length === 0 ? (<div className="py-16 text-center text-[13px] text-gray-400">Bu guruhda o&apos;quvchi yo&apos;q</div>) : (<div className="overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                ...
                return (<th key={d} className={cn("px-0 py-3 text-center text-[11px] font-medium min-w-[40px] w-10", weekend ? "text-gray-300 dark:text-gray-600" : "text-gray-400 dark:text-gray-500", isToday && "text-[#5E2CA5] dark:text-purple-400 font-bold")}>
                        {day}
                      </th>);
            })}
