...
export const PhoneInput = forwardRef(function PhoneInput({ value, onChange, onBlur, id, name, label, error, placeholder = "94 071 35 55", className, inputClassName, disabled, required, autoFocus, size = "sm" }, forwardedRef) {
    const [digits, setDigits] = useState(() => toNationalDigits(value || ""));
    const lastEmitted = useRef(value || "");
    const caretDigitTarget = useRef(null);
    ...
    useLayoutEffect(() => {
        if (caretDigitTarget.current === null)
            return;
    ...
    const handleChange = (e) => {
        ...
        if (extracted.length > 9 && extracted.startsWith("998"))
    ...
    const handleBeforeInput = (e) => {
        ...
        if (!data || data.length !== 1)
            return; // let paste/autofill through — onChange normalizes it
        if (/\D/.test(data)) {
            e.preventDefault();
            return;
        }
    ...
    return (<div className="w-full">
      {label && (<label htmlFor={id} className={cn("block text-sm font-medium mb-1.5", size === "lg" ? "text-gray-700" : "text-gray-700 dark:text-gray-300")}>
          {label}
        </label>)}
      <div className={cn("flex items-center border rounded-xl transition", size === "lg"
            ? "bg-white border-gray-300 focus-within:ring-2 focus-within:ring-[#5E2CA5] focus-within:border-transparent"
            : "bg-white dark:bg-gray-800 border-gray-200 dark:border-white/10 focus-within:ring-2 focus-within:ring-[#5E2CA5]/30 focus-within:border-[#5E2CA5]/50", error && (size === "lg" ? "border-red-400" : "border-red-400 dark:border-red-600"), disabled && "opacity-60", className)}>
        <span className={cn("select-none mr-2 flex-shrink-0", size === "lg"
        ...
        }} id={id} name={name} type="tel" inputMode="numeric" autoComplete="tel-national" value={formatNational(digits)} onChange={handleChange} onBeforeInput={handleBeforeInput} onBlur={onBlur} placeholder={placeholder} disabled={disabled} required={required} autoFocus={autoFocus} className={cn("flex-1 min-w-0 bg-transparent placeholder-gray-400 outline-none", size === "lg" ? "py-3 pr-4 text-base text-gray-900" : "py-2 pr-3 text-[13px] text-gray-900 dark:text-white", inputClassName)}/>
      </div>
