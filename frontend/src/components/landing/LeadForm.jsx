...
export function LeadForm({ selectedCourse }) {
    ...
    useEffect(() => {
        if (selectedCourse)
            setForm((f) => ({ ...f, courseName: selectedCourse }));
    }, [selectedCourse]);
    ...
    const inp = "w-full px-4 py-3 text-base border border-white/10 rounded-xl bg-white/5 text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-violet-accent focus:border-transparent transition";
    const label = "flex items-center gap-1.5 text-sm font-medium text-white/70 mb-1.5";
    return (<section id="ariza" className="border-b border-white/10">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-20 sm:py-28">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <Reveal>
            <span className="landing-eyebrow">{t.formLabel}</span>
              ...
              {done ? (<div className="text-center py-6 sm:py-8">
                ...
                setForm(EMPTY_FORM);
                  ...
                  <div>
                    ...
                    <input className={inp} value={form.fullName} onChange={(e) => setForm((f) => ({ ...f, fullName: e.target.value }))} placeholder={t.formNamePlaceholder}/>
                  </div>
                  <div>
                    <label className={label}>
                      <Phone className="w-3.5 h-3.5 text-violet-accent"/> {t.formPhone} *
                    </label>
                    <PhoneInput value={form.phone} onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value }))} size="lg" className="!bg-white/5 !border-white/10" inputClassName="!text-white placeholder:!text-white/30"/>
                  </div>
                  <div>
                    <label className={label}>
                    ...
                    <select className={inp} value={form.courseName} onChange={(e) => setForm((f) => ({ ...f, courseName: e.target.value }))}>
                      ...
                      {courses.map((c) => (<option className="bg-[#08060F]" key={c.name} value={c.name}>
                          {c.name}
                        </option>))}
                    </select>
                  </div>
                  <div>
                    ...
                    <textarea className={inp} rows={3} value={form.note} onChange={(e) => setForm((f) => ({ ...f, note: e.target.value }))} placeholder={t.formCommentPlaceholder}/>
                  </div>
