import { Sparkles, Lightbulb, Package, Code2, Rocket, Zap, TrendingUp, ArrowRight } from "lucide-react";
...
export function StartupStudio({ onSelect }) {
    const { lang, t } = useLang();
    const highlights = STUDIO_CARDS[lang];
    const stages = TIMELINE[lang];
    return (<section id="startup-studio" className="relative border-b border-white/10 overflow-hidden">
      <div aria-hidden="true" className="pointer-events-none absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[600px] rounded-full bg-violet-accent/10 blur-[120px]"/>

      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-20 sm:py-28">
        <Reveal className="max-w-2xl mx-auto text-center">
          <span className="landing-eyebrow inline-flex items-center gap-1.5">
            <Sparkles className="w-3.5 h-3.5"/> Startup Studio <Sparkles className="w-3.5 h-3.5"/>
          </span>
          <h2 className="landing-heading mt-3">Startup Studio</h2>
        </Reveal>
        ...
        <div className="mt-6 grid sm:grid-cols-2 gap-5 max-w-3xl mx-auto">
          {highlights.map((h, i) => {
            ...
            return (<Reveal key={h.title} delay={180 + i * 80}>
                <div className="landing-card h-full p-6 flex items-start gap-4">
        ...
        <div className="mt-14 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {stages.map((title, i) => {
            ...
            return (<Reveal key={title} delay={380 + i * 80}>
                <div className="relative">
                  ...
                  {i < stages.length - 1 && (<ArrowRight className="hidden lg:block absolute top-1/2 -right-5 -translate-y-1/2 w-4 h-4 text-violet-accent/40"/>)}
                </div>
              </Reveal>);
        })}
        </div>
        ...
        <Reveal delay={700}>
          <div className="mt-14 text-center">
            <button type="button" onClick={onSelect} className="btn-primary px-8 py-3.5">
