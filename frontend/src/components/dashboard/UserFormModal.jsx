...
export default function UserFormModal({ user, onClose, onSuccess }) {
    ...
    return (<Modal title={user ? "Hodimni tahrirlash" : "Yangi hodim"} maxWidth="max-w-lg" onClose={onClose}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                <input {...register("password")} type={showPass ? "text" : "password"} placeholder="Kamida 8 belgi" className="w-full px-3 py-2 text-sm border border-gray-200 dark:border-white/10 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30 focus:border-[#5E2CA5]/50 transition"/>
            ...
            {generatedPass && (<p className="mt-1 text-xs text-emerald-600 dark:text-emerald-400 font-mono bg-emerald-50 dark:bg-emerald-900/20 px-2 py-1 rounded-lg">
          ...
          <div>
            ...
            <select value={watch("role")} onChange={(e) => applyRoleDefaults(e.target.value)} className="w-full px-3 py-2 text-sm border border-gray-200 dark:border-white/10 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30 focus:border-[#5E2CA5]/50 transition">
          ...
          <div>
            ...
            <div className="space-y-2">
              {permissions.map((p) => (<label key={p.key} className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" {...register(p.key)} className="h-4 w-4 accent-[#5E2CA5] border-gray-300 rounded"/>
