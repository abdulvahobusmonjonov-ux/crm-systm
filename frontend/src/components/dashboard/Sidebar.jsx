...
import { LayoutDashboard, Users, BookOpen, Bell, BarChart3, Settings, LogOut, ChevronLeft, ChevronRight, ChevronDown, X, GraduationCap, UserCog, Pin, Wallet, TrendingUp, Presentation, CalendarRange, TrendingDown, PlusCircle, Banknote, ArrowLeftRight, ClipboardList, Trophy, ScrollText, FileBarChart, AlertCircle, Gift, UserCheck, CheckSquare, Briefcase, ShieldCheck, } from "lucide-react";
import { cn, getInitials } from "@/lib/utils";
import { apiFetch, LOGO_URL } from "@/lib/api";
const isAdminRole = (u) => u.role === "SUPER_ADMIN" || u.role === "ADMIN";
const isManagerRole = (u) => isAdminRole(u) || u.role === "MANAGER";
...
const isActivePath = (href, pathname) => pathname === href || (href !== "/dashboard" && pathname.startsWith(href));
const hasActiveChild = (children, pathname) => children.some((c) => isActivePath(c.href, pathname));
const initOpenGroups = (pathname) => {
    const result = {};
...
export default function Sidebar({ mobileOpen = false, onClose = () => { } }) {
    ...
    const [openGroups, setOpenGroups] = useState(() => initOpenGroups(pathname));
    useEffect(() => {
    ...
    const topLinkCls = (href) => cn("flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium transition-colors duration-150", isActivePath(href, pathname)
    ...
    const childLinkCls = (href) => {
        const active = isActivePath(href, pathname);
        return cn("flex items-center gap-3 pl-2.5 pr-3 py-2 rounded-lg text-[13px] font-medium transition-colors duration-150 border-l-2", active
    ...
    const renderNavItem = (item, inGroup = false) => {
        ...
        return (<Link key={item.href} href={item.href} title={collapsed ? item.label : undefined} className={inGroup ? childLinkCls(item.href) : topLinkCls(item.href)}>
        <Icon className={ICON}/>
    ...
    const renderNavEntry = (entry) => {
        if (entry.type === "group") {
            ...
            if (collapsed) {
                return (<div key={entry.label} className="mt-3 pt-3 border-t border-gray-100 dark:border-white/5">
          ...
          <button onClick={() => toggle(entry.label)} className={cn("w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium transition-colors duration-150", activeInGroup
                    ? "bg-[#5E2CA5] text-white shadow-sm"
                    : "text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5 hover:text-gray-900 dark:hover:text-white")}>
            <Icon className={ICON}/>
          ...
          {open && (<div className="mt-1 space-y-0.5 ml-1">
        ...
        const Icon = entry.icon;
        if (!entry.children) {
            return (<Link key={entry.href} href={entry.href} title={collapsed ? entry.label : undefined} className={topLinkCls(entry.href)}>
          <Icon className={ICON}/>
        ...
        <div className={cn("flex items-center", !collapsed && "gap-0.5")}>
          <Link href={entry.href} title={collapsed ? entry.label : undefined} className={cn(topLinkCls(entry.href), "flex-1")}>
            <Icon className={ICON}/>
            {!collapsed && <span>{entry.label}</span>}
          </Link>
          {!collapsed && (<button onClick={() => toggle(entry.label)} className="p-2 rounded-xl text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5 hover:text-gray-600 dark:hover:text-gray-300 transition-colors duration-150">
              <ChevronDown className="w-3.5 h-3.5" style={chevronStyle(open)}/>
            </button>)}
        </div>
        {open && !collapsed && (<div className="mt-1 space-y-0.5 ml-1 pl-3 border-l-2 border-gray-100 dark:border-white/5">
            {entry.children.map((item) => renderNavItem(item, true))}
          </div>)}
      </div>);
    };
    return (<>
      ...
      <div className="border-t border-gray-100 dark:border-white/5 p-3">
        <div className={cn("flex items-center gap-2.5", collapsed && "justify-center")}>
          <Link href="/profile" title="Profil" className={cn("flex items-center gap-2.5 min-w-0 flex-1 rounded-xl -m-1 p-1 transition-colors duration-150 hover:bg-gray-100 dark:hover:bg-white/5", collapsed && "flex-initial justify-center")}>
