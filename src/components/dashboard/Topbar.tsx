...
export default function Topbar({ onMenuClick }: { onMenuClick?: () => void }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(`/api/leads?search=${encodeURIComponent(q)}&limit=8`, { signal: controller.signal });
        if (res.ok) {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onMenuClick}
        className="lg:hidden p-2 rounded-xl text-gray-500 hover:bg-gray-100 dark:hover:bg-white/5 transition-colors flex-shrink-0"
        aria-label="Menyu"
      >
        ...
        <div className="relative w-full max-w-xs lg:max-w-sm" ref={searchBoxRef}>
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-2 text-[13px] border border-gray-200 dark:border-white/10 rounded-xl bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30 focus:border-[#5E2CA5]/50 transition"
            />
            {searching && (
              <div className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 border-2 border-gray-200 dark:border-white/10 border-t-[#5E2CA5] rounded-full animate-spin" />
            )}
          </div>

          {searchOpen && search.trim().length >= 2 && (
            <div className="absolute left-0 right-0 top-full mt-2 bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-100 dark:border-white/10 z-50 overflow-hidden py-1.5 max-h-96 overflow-y-auto">
      ...
      <div className="flex items-center gap-2 sm:gap-3 ml-auto sm:ml-0">
        ...
        <Link
          href="/leads/new"
          className="hidden md:flex items-center gap-1.5 px-3 py-1.5 bg-[#5E2CA5] hover:bg-[#4a2280] text-white text-[13px] font-medium rounded-xl transition-colors flex-shrink-0"
        >
        ...
        <Link
          href="/leads/new"
          className="md:hidden flex items-center justify-center w-9 h-9 bg-[#5E2CA5] hover:bg-[#4a2280] text-white rounded-xl transition-colors flex-shrink-0"
        ...
        <Link
          href="/timetable"
          className="p-2.5 rounded-xl text-gray-500 hover:bg-[#5E2CA5]/10 hover:text-[#5E2CA5] dark:hover:bg-[#5E2CA5]/20 dark:hover:text-purple-300 transition-colors flex-shrink-0"
          title="Dars jadvali"
        ...
        <button
          onClick={toggleDark}
          className="p-2 rounded-xl text-gray-500 hover:bg-gray-100 dark:hover:bg-white/5 transition-colors flex-shrink-0"
        ...
        <div className="relative flex-shrink-0" ref={userMenuRef}>
          <button
            ...
            className="flex items-center gap-2 p-1 pr-1.5 sm:pr-2.5 rounded-xl hover:bg-gray-100 dark:hover:bg-white/5 transition-colors"
          >
            <div className="w-8 h-8 rounded-full bg-[#5E2CA5]/10 dark:bg-[#5E2CA5]/20 flex items-center justify-center flex-shrink-0 overflow-hidden">
              {user?.avatarUrl ? (
                ...
                <span className="text-[11px] font-semibold text-[#5E2CA5] dark:text-purple-300">
          ...
          {userMenuOpen && (
            <div className="absolute right-0 top-full mt-2 w-52 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-white/10 z-50 overflow-hidden py-1.5">
              ...
              <Link
                ...
                className="flex items-center gap-2.5 px-3.5 py-2 text-[13px] text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
              >
