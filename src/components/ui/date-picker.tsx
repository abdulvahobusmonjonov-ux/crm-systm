...
import {
  ...
  eachDayOfInterval, isSameDay, isSameMonth, isToday as isTodayFn,
...
const triggerCls =
  "inline-flex items-center justify-between gap-2 px-3 py-2 text-[13px] border rounded-xl bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30 focus:border-[#5E2CA5]/50 transition border-gray-200 dark:border-white/10";

const popupCls =
  "bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-100 dark:border-white/10 p-3";

const navBtnCls =
  "p-1.5 rounded-lg hover:bg-[#5E2CA5]/10 text-gray-500 dark:text-gray-400 hover:text-[#5E2CA5] transition-colors";

const footerBtnCls =
  "flex-1 py-1.5 rounded-lg text-[12px] font-medium text-[#5E2CA5] hover:bg-[#5E2CA5]/10 transition-colors";

function PopupFooter({ onToday, onClear }: { onToday: () => void; onClear: () => void }) {
  return (
...
export function DatePicker({ value, onChange, className, placeholder = "Sana tanlang", disabled, id }: DatePickerProps) {
  ...
  useEffect(() => {
    if (open) setViewDate(selectedValid ?? new Date());
  ...
  return (
    <>
      <button
        ...
        disabled={disabled}
        onClick={() => setOpen((o) => !o)}
        className={cn(triggerCls, disabled && "opacity-60 cursor-not-allowed", className)}
      >
        ...
        createPortal(
          ...
          >
            ...
            <div className="grid grid-cols-7 gap-1 mb-1">
              {WEEKDAYS_UZ.map((w) => (
                <div key={w} className="text-center text-[11px] font-medium text-gray-400 dark:text-gray-500 py-1">
                  {w}
                </div>
              ))}
            </div>
              ...
              {days.map((d) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => select(d)}
                    className={cn(
                      "h-8 w-8 text-[12px] rounded-lg transition-colors flex items-center justify-center",
                      isSel
                        ...
                        : inMonth
                          ...
                          : "text-gray-300 dark:text-gray-600 hover:bg-[#5E2CA5]/10",
                      !isSel && isTod && "ring-1 ring-[#5E2CA5]/50 font-semibold"
                    )}
                  >
                    {d.getDate()}
                  </button>
...
export function MonthPicker({ value, onChange, className, placeholder = "Oy tanlang", disabled, id }: MonthPickerProps) {
  const [open, setOpen] = useState(false);
  const triggerRef = useRef<HTMLButtonElement>(null);
  const popupRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    if (open) setViewYear(selectedValid?.getFullYear() ?? new Date().getFullYear());
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open]);
              ...
              {MONTHS_UZ.map((m, i) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => select(i)}
                    className={cn(
                      "py-2 rounded-lg text-[12px] transition-colors",
                      isSel
                        ? "bg-[#5E2CA5] text-white font-semibold"
                        : "text-gray-700 dark:text-gray-200 hover:bg-[#5E2CA5]/10 hover:text-[#5E2CA5] dark:hover:text-white"
                    )}
