...
export function EmptyState({ icon: Icon = Inbox, title, description, action, className }: EmptyStateProps) {
  return (
    <div
      className={cn(
        "bg-white dark:bg-gray-900 rounded-2xl shadow-sm py-16 px-6 text-center",
        className
      )}
    ...
    >
      ...
      {description && (
        <p className="text-[13px] text-gray-400 dark:text-gray-500 mt-1.5 max-w-xs mx-auto">{description}</p>
      )}
