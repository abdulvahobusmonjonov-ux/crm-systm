...
import { Users, Wallet, LogOut, ChevronLeft, ChevronRight, X } from "lucide-react";
import { cn, getInitials } from "@/lib/utils";

// Server layouts (no "use client") can't pass component references as props to this
// client component — only serializable data crosses that boundary. Nav items carry an
// icon *name* instead, resolved to a component here.
const PORTAL_ICONS = { users: Users, wallet: Wallet } as const;
export type PortalIconName = keyof typeof PORTAL_ICONS;
export interface PortalNavItem { href: string; label: string; icon: PortalIconName; }
...
export default function PortalSidebar({ navItems, portalLabel, profileHref, mobileOpen = false, onClose = () => {} }: PortalSidebarProps) {
    ...
    <>
      ...
      >
        <div className={cn(
          ...
          <img src={logo || "/api/branding/logo"} alt="Logo" className="w-8 h-8 rounded-xl object-contain flex-shrink-0" />
        ...
        <nav className="flex-1 px-2.5 py-4 overflow-y-auto space-y-0.5 scrollbar-none">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} title={collapsed ? item.label : undefined} className={linkCls(item.href)}>
                <Icon className={ICON} />
            ...
            >
              ...
              {!collapsed && (
                <div className="min-w-0 flex-1">
                  <p className="text-[13px] font-medium text-gray-900 dark:text-white truncate leading-tight">{user?.fullName ?? user?.name}</p>
                  <p className="text-[11px] text-gray-400 dark:text-gray-500 truncate">{portalLabel}</p>
                </div>
              )}
            </Link>
