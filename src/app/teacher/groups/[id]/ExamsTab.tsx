...
export default function ExamsTab({ groupId }: { groupId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(false);
    try {
      const res = await fetch(`/api/teacher/groups/${groupId}/exams`);
      if (!res.ok) { setError(true); setLoading(false); return; }
      const data = await res.json();
      setExams((data.exams || []).map((e: Exam) => ({ ...e, date: e.date.slice(0, 10) })));
    } catch {
      setError(true);
    }
    setLoading(false);
  ...
  const create = async () => {
    if (!form.title.trim() || !form.date) { toast.error("Nomi va sanani kiriting"); return; }
    setCreating(true);
    const res = await fetch(`/api/teacher/groups/${groupId}/exams`, {
  ...
  const remove = async (id: string) => {
    if (!(await confirm({ title: "Imtihonni o'chirish", message: "Bu imtihonni o'chirasizmi?" }))) return;
    const res = await fetch(`/api/teacher/groups/${groupId}/exams/${id}`, { method: "DELETE" });
    ...
    <div className="space-y-3">
      ...
      <Card className="overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center text-[13px] text-gray-400">Hali imtihon qo&apos;shilmagan</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody>
                {exams.map((e) => (
                  <tr key={e.id} className="border-b last:border-0 border-gray-50 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
