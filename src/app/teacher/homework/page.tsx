...
export default function TeacherHomeworkPage() {
  ...
  const load = useCallback(async () => {
    ...
    const data = await fetch(`/api/teacher/homework?${params}`).then((r) => r.json());
  ...
  const saveGrade = async (markChecked: boolean) => {
    if (!grading) return;
    setSaving(true);
    const res = await fetch(`/api/teacher/homework/${grading.id}`, {
  ...
  return (
    <div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
          ...
          {(["all", "unchecked", "checked"] as const).map((s) => (
            <button
              ...
              className={cn(
                ...
                status === s ? "bg-white dark:bg-gray-800 text-[#5E2CA5] shadow-sm" : "text-gray-500 dark:text-gray-400"
              )}
            >
              ...
              <div className="divide-y divide-gray-50 dark:divide-white/5">
                {ex.submissions.map((s) => (
                  <button
                    key={s.id}
                    onClick={() => openGrade(s)}
                    className="w-full flex items-center gap-3 px-5 py-3 text-left hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
                  >
                    ...
                    {s.grade !== null && (
                      <span className="text-[12px] font-bold text-gray-700 dark:text-gray-300 flex-shrink-0">{s.grade}</span>
                    )}
                    {s.status === "checked" ? (
                      <span className="flex items-center gap-1 text-[11px] font-semibold text-emerald-600 dark:text-emerald-400 flex-shrink-0">
                        <CheckCircle2 className="w-3.5 h-3.5" /> Tekshirilgan
                      </span>
                    ) : (
                      <span className="flex items-center gap-1 text-[11px] font-semibold text-amber-600 dark:text-amber-400 flex-shrink-0">
      ...
      {grading && (
        <Modal
          title={grading.lead.fullName}
          subtitle="Topshiriqni baholash"
        ...
        >
          {grading.content && (
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-white/5 text-[13px] text-gray-700 dark:text-gray-300 whitespace-pre-wrap">
              {grading.content}
            </div>
          )}
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 text-[13px] border border-gray-200 dark:border-white/10 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30 focus:border-[#5E2CA5]/50 transition"
            />
          </div>
          <div>
            ...
            <textarea
              ...
              onChange={(e) => setGradeForm((f) => ({ ...f, teacherNote: e.target.value }))}
              rows={3}
              className="w-full px-3 py-2 text-[13px] border border-gray-200 dark:border-white/10 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30 focus:border-[#5E2CA5]/50 transition resize-none"
            />
          </div>
        </Modal>
      )}
