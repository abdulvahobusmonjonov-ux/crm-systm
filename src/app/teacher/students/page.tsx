...
export default function TeacherStudentsPage() {
  ...
  const load = useCallback(async (q: string) => {
    ...
    const data = await fetch(`/api/teacher/students?${params}`).then((r) => r.json());
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody>
                {students.map((s) => (
                  <tr key={s.id} className="border-b last:border-0 border-gray-50 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                    ...
                    {showContacts && (
                      <td className="px-5 py-3.5 hidden sm:table-cell text-[13px] text-gray-500 dark:text-gray-400">{s.phone ? formatPhone(s.phone) : "—"}</td>
                    )}
