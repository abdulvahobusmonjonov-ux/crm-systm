...
interface TeacherStat { name: string; groups: number; students: number; present: number; total: number; revenue: number; attendance: number; averageScore?: number | null; }

// Derived star rating from attendance % (no separate rating data exists)
function ratingFromAttendance(pct: number): number {
  if (pct >= 90) return 5;
  if (pct >= 80) return 4;
  if (pct >= 70) return 3;
  if (pct >= 60) return 2;
...
export default function TeacherReportPage() {
  const [list, setList] = useState<TeacherStat[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/reports/teacher-performance")
      .then(r => r.json())
  ...
  const exportCSV = async () => {
    ...
    if (res.ok) {
      ...
      a.href = url; a.download = `oqituvchi_hisoboti_${new Date().toISOString().slice(0, 10)}.xlsx`;
    ...
    <div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {kpis.map(k => {
          const Icon = k.icon;
          return (
            <div key={k.label} className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-5 flex items-center gap-4">
                  ...
                  return (
                    <tr key={i} className="hover:bg-gray-50/60 dark:hover:bg-white/5 transition-colors duration-100">
                      ...
                      <td className="px-4 py-3 text-right">
                        <span className={cn(
                          "inline-flex items-center text-[12px] font-bold px-2.5 py-1 rounded-full tabular-nums",
                          pctGood
