...
interface User {
  ...
  email: string | null;
  phone: string | null;
  role: "SUPER_ADMIN" | "ADMIN" | "MANAGER" | "OPERATOR" | "MENTOR" | "RECEPTION" | "ACCOUNTANT";
  isActive: boolean;
  createdAt: string;
...
export default function UsersPage() {
  ...
  const handleDelete = async (user: User) => {
    if (!(await confirm({ title: "Hodimni o'chirish", message: `${user.fullName} ni o'chirishni tasdiqlaysizmi?` }))) return;
    ...
    <div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                {filtered.map((user) => (
                  <tr key={user.id} className="hover:bg-gray-50/60 dark:hover:bg-white/5 transition-colors duration-100">
      ...
      {modalOpen && (
        <UserFormModal
          ...
          onSuccess={() => { setModalOpen(false); setEditUser(null); loadUsers(); }}
        />
      )}
    </div>
  );
