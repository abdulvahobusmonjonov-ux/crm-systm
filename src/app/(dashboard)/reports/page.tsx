...
const ReportsChart = dynamic(() => import("./ReportsChart"), {
  ssr: false,
  loading: () => <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm h-72 animate-pulse" />,
});
...
interface ReportsData {
  ...
  courseStats: { courseId: string | null; course: { name: string; color: string } | null; count: number }[];
  userStats: { id: string; fullName: string; totalLeads: number; createdLeads: number }[];
}
  ...
  return (
    <div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {loading ? (
        ...
        <div className="text-center text-gray-400 py-20 text-sm">Ma&apos;lumot yuklanmadi</div>
      ) : (
        <>
            ...
            return (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {kpis.map(k => {
                  const Icon = k.icon;
                  return (
                    <div key={k.label} className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-5">
              ...
              <div className="space-y-3.5">
                {(() => {
                  const total = (data.sourceDistribution || []).reduce((a: number, s) => a + s.count, 0) || 1;
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-5">
              ...
              <div className="space-y-3.5">
                {(() => {
                  ...
                  const total = stages.reduce((a: number, s) => a + s.count, 0) || 1;
                  return stages.map((s) => {
                    const pct = Math.round((s.count / total) * 100);
                    return (
                      <div key={s.stageId}>
                        ...
                        <div className="w-full bg-gray-100 dark:bg-white/8 rounded-full h-2 overflow-hidden">
                          <div
                            className="h-2 rounded-full transition-all duration-300"
                            style={{ width: `${pct}%`, backgroundColor: s.color || "#9ca3af" }}
                          />
                        </div>
                      </div>
                    );
