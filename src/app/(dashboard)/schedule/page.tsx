...
interface Slot { id: string; label: string; startTime: string; isActive?: boolean; }
interface Lead {
  id: string; fullName: string; status: string;
  timeSlotId: string | null; preferredDays: string | null;
...
export default function SchedulePage() {
  ...
  const [selectedCell, setSelectedCell] = useState<{ slotId: string; dayIndex: number } | null>(null);
  const [cellLeads, setCellLeads] = useState<Lead[]>([]);

  useEffect(() => {
    Promise.all([
      fetch("/api/timeslots").then(r => r.json()),
      fetch("/api/leads?limit=200").then(r => r.json()).then(d => d.leads),
    ]).then(([s, l]) => { setSlots(s); setLeads(l); setLoading(false); });
  }, []);

  const weekDays = Array.from({ length: 7 }, (_, i) => addDays(weekStart, i));

  const getLeadsForCell = (slotId: string, dayIndex: number): Lead[] => {
    const dayName = DAYS_EN[dayIndex];
    return leads.filter(l => l.timeSlotId === slotId || l.preferredDays?.includes(dayName));
  };

  const maxCount = Math.max(1, ...slots.flatMap(slot =>
    weekDays.map((_, i) => getLeadsForCell(slot.id, i).length)
  ));

  const handleCellClick = (slotId: string, dayIndex: number) => {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-3 py-2 text-sm rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:text-[#5E2CA5] hover:border-[#5E2CA5]/30 transition-colors shadow-sm"
            >
              Bugun
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2 space-y-3">
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
              {loading ? (
                <div className="p-10 text-center text-gray-400 animate-pulse text-sm">Yuklanmoqda...</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-100 dark:border-white/5">
                        ...
                        {weekDays.map((day, i) => {
                          ...
                          return (
                            <th key={i} className="px-2 py-3 text-center min-w-[56px]">
                    ...
                    <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                      {slots.filter(s => s.isActive !== false).length === 0 ? (
                        <tr>
                          <td colSpan={8} className="px-4 py-10 text-center text-sm text-gray-400">
                            Hali vaqt slotlari yo&apos;q. Sozlamalar &rarr; Vaqt slotlari bo&apos;limidan qo&apos;shing.
                          </td>
                        </tr>
                      ) : slots.filter(s => s.isActive !== false).map(slot => (
                        <tr key={slot.id} className="hover:bg-gray-50/50 dark:hover:bg-white/2 transition-colors">
                          ...
                          {weekDays.map((_, dayIdx) => {
                            ...
                            return (
                              <td key={dayIdx} className="px-1.5 py-2 text-center">
                                <button
                                  onClick={() => handleCellClick(slot.id, dayIdx)}
                                  className={[
            ...
            <div className="flex items-center gap-3 px-1">
              ...
              {(["Kam", "O'rta", "Ko'p", "Juda ko'p"] as const).map((label, i) => (
                ...
                <div className="space-y-1.5">
                  ...
                  {cellLeads.map(l => (
                    <Link
                      key={l.id}
                      href={`/leads/${l.id}`}
                      className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-[#5E2CA5]/5 dark:hover:bg-[#5E2CA5]/10 transition-colors group"
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {l.course && (
                          <p className="text-[11px] text-gray-400 truncate">{l.course.name}</p>
                        )}
                      </div>
                    </Link>
                  ))}
