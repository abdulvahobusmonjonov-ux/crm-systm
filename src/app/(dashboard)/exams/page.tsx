...
import {
  Plus, Trash2, ArrowLeft, Save, ClipboardList, BookOpen,
...
function grade(score: number | null | undefined, max: number): string {
...
function examStatus(e: Exam): { label: string; cls: string } {
...
export default function ExamsPage() {
  ...
  const create = async () => {
    ...
    setSaving(false);
    if (res.ok) { toast.success("Yaratildi"); setShowAdd(false); setForm({ title: "", type: "exam", groupId: "", courseId: "", date: ymd(new Date()), maxScore: "100" }); load(); }
    else toast.error("Saqlab bo'lmadi");
  };

  const del = async (id: string) => {
    if (!(await confirm({ title: "Imtihonni o'chirish", message: "Bu imtihon va uning ballarini o'chirasizmi?" }))) return;
    const res = await fetch(`/api/exams/${id}`, { method: "DELETE" });
    if (res.ok) { toast.success("O'chirildi"); load(); } else toast.error("O'chirib bo'lmadi");
  };

  if (selected) return <ScoreEntry exam={selected} onBack={() => { setSelected(null); load(); }} dialog={dialog} />;
          ...
          {exams.map(e => {
            ...
            return (
              <div
                ...
                className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm hover:shadow-md transition-shadow duration-200 cursor-pointer flex flex-col overflow-hidden group"
              >
                <div className="p-5 flex-1">
                  ...
                  <div className="flex items-start justify-between gap-2 mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="p-1 text-gray-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors opacity-0 group-hover:opacity-100"
                      >
                  ...
                  {/* Exam type badge */}
                  <span className="inline-block text-[10px] font-semibold uppercase tracking-wide mb-1.5 px-2 py-0.5 rounded-full"
                  ...
                  {e.group && (
                    <p className="text-[12px] text-gray-400 mt-0.5 truncate">{e.group.name}</p>
                  )}
                </div>
...
/* ─────────────── ScoreEntry ─────────────── */
function ScoreEntry({ exam, onBack, dialog }: { exam: Exam; onBack: () => void; dialog: ReactNode }) {
  ...
  useEffect(() => {
    fetch(`/api/exams/${exam.id}/scores`).then(r => r.json()).then(d => {
  ...
  const setScore = (leadId: string, v: string) =>
    setStudents(prev => prev.map(s => s.leadId === leadId ? { ...s, score: v === "" ? null : Number(v) } : s));

  const save = async () => {
    const records = students
      .filter(s => s.score !== null && s.score !== undefined && s.score !== "")
      .map(s => ({ leadId: s.leadId, score: Number(s.score) }));
    if (!records.length) { toast.error("Hech bo'lmasa bittasini kiriting"); return; }
    setSaving(true);
    ...
    setSaving(false);
    if (res.ok) toast.success("Ballar saqlandi"); else toast.error("Saqlab bo'lmadi");
  };
  ...
  return (
    <div className="min-h-screen bg-[#F5F6FA] dark:bg-gray-950 p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm py-16 text-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="divide-y divide-gray-50 dark:divide-white/5">
              {students.map(s => (
                <div key={s.leadId} className="flex items-center justify-between gap-3 px-5 py-2.5 hover:bg-gray-50/60 dark:hover:bg-white/5 transition-colors">
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <input
                      ...
                      value={s.score ?? ""}
                      onChange={e => setScore(s.leadId, e.target.value)}
                      placeholder="—"
                      className="w-20 px-2.5 py-1.5 text-[13px] text-right border border-gray-200 dark:border-white/10 rounded-xl bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30 focus:border-[#5E2CA5]/50 transition"
                    />
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
            ...
            {ranked.length === 0 ? (
              <div className="py-12 text-center text-[13px] text-gray-400">Hali ball kiritilmagan</div>
            ) : (
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                    {ranked.map((s, idx) => {
                      ...
                      return (
                        <tr key={s.leadId} className="hover:bg-gray-50/60 dark:hover:bg-white/5 transition-colors">
                          {/* Rank */}
                          <td className="px-4 py-2.5 text-center">
                            {idx === 0 ? (
                              ...
                              <span className="text-[12px] font-bold text-gray-400 tabular-nums">{idx + 1}</span>
                            )}
                          </td>
                          {/* Student */}
                    ...
                    {/* Unscored students */}
                    {unscored.map(s => (
                      <tr key={s.leadId} className="opacity-40 hover:bg-gray-50/60 dark:hover:bg-white/5">
                        <td className="px-4 py-2.5 text-center">
