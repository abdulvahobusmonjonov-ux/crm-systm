...
const TYPE_META: Record<ChangeType, { label: string; icon: typeof Sparkles; cls: string }> = {
      ...
      <div className="max-w-2xl">
        <ol className="relative">
          {CHANGELOG.map((entry, idx) => {
            ...
            return (
              <li key={entry.version} className="relative pl-10 pb-6 last:pb-0">
                {/* Vertical line */}
                {!isLast && (
                  <span
                    className="absolute left-[13px] top-7 bottom-0 w-px bg-gray-200 dark:bg-white/10"
                    aria-hidden
                  />
                )}
                {/* Dot */}
                <span
                  className="absolute left-0 top-1 w-7 h-7 rounded-full flex items-center justify-center ring-4 ring-[#F5F6FA] dark:ring-gray-950"
                  style={{ backgroundColor: idx === 0 ? BRAND : "#E5E7EB" }}
                >
                  <span
                    className={cn("w-2.5 h-2.5 rounded-full", idx === 0 ? "bg-white" : "bg-gray-400 dark:bg-gray-600")}
                  />
                </span>
                ...
                <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-4 sm:p-5">
                  <div className="flex items-center flex-wrap gap-2 mb-2">
                    <span
                      className="inline-flex items-center text-[12px] font-bold px-2.5 py-1 rounded-full text-white"
