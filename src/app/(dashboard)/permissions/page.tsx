...
import { PERMISSION_CATALOG, TIER_LABELS, type PermissionTier } from "@/lib/permission-catalog";
import { cn, getInitials } from "@/lib/utils";

interface StaffUser { id: string; fullName: string; username: string; role: string; }

const TIER_BADGE_CLS: Record<PermissionTier, string> = {
...
const MODULES = (() => {
  const byModule = new Map<string, { module: string; moduleLabel: string; actions: typeof PERMISSION_CATALOG }>();
  for (const p of PERMISSION_CATALOG) {
    if (!byModule.has(p.module)) byModule.set(p.module, { module: p.module, moduleLabel: p.moduleLabel, actions: [] });
...
export default function PermissionsPage() {
  ...
  const toggle = (module: string, action: string, tier: PermissionTier) => {
    ...
    if (res.ok) {
      ...
      setGranted((prev) => {
        ...
        for (const g of grants) {
          ...
          if (g.granted) next.add(k); else next.delete(k);
        }
        return next;
      });
      setDirty(new Map());
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-4">
          ...
          <div className="p-3 border-b border-gray-100 dark:border-white/5">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 text-[13px] border border-gray-200 dark:border-white/10 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#5E2CA5]/30 focus:border-[#5E2CA5]/50 transition"
              />
            </div>
          </div>
              ...
              filtered.map((u) => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedId === u.id ? "bg-[#5E2CA5]/8 dark:bg-[#5E2CA5]/15" : "hover:bg-gray-50 dark:hover:bg-white/5"
                  )}
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[11px] text-gray-400 truncate">{ROLE_LABELS[u.role as keyof typeof ROLE_LABELS] || u.role}</p>
                  </div>
                </button>
              ))
            )}
        ...
        <Card className="overflow-hidden">
          {!selectedId ? (
            ...
            <div className="py-20 text-center text-[13px] text-gray-400 animate-pulse">Yuklanmoqda...</div>
          ) : (
            <>
              <div className="px-5 py-4 border-b border-gray-100 dark:border-white/5 flex items-center justify-between gap-3 flex-wrap">
                <div>
                  ...
                  <p className="text-[12px] text-gray-400">{ROLE_LABELS[selectedUser?.role as keyof typeof ROLE_LABELS] || selectedUser?.role}</p>
                </div>
                <button
                  onClick={save}
                  disabled={saving || dirty.size === 0}
                  className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-semibold bg-[#5E2CA5] hover:bg-[#4a2280] text-white shadow-sm transition-colors disabled:opacity-50"
                >
              ...
              <div className="max-h-[calc(100vh-260px)] overflow-y-auto divide-y divide-gray-50 dark:divide-white/5">
                {MODULES.map((m) => (
                  <div key={m.module} className="px-5 py-4">
                          ...
                          >
                            <span className="flex items-center gap-2 min-w-0">
                              <input
                                ...
                                onChange={() => toggle(a.module, a.action, a.tier)}
                                className="h-4 w-4 accent-[#5E2CA5] rounded flex-shrink-0"
                              />
                              <span className="text-[13px] text-gray-700 dark:text-gray-300 truncate">{a.label}</span>
                            </span>
                            {a.tier !== "ready" && (
                              <span className={cn("text-[10px] font-semibold px-1.5 py-0.5 rounded-full flex-shrink-0", TIER_BADGE_CLS[a.tier])}>
                                {TIER_LABELS[a.tier]}
                              </span>
                            )}
                          </label>
