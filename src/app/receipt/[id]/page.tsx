...
interface Receipt {
  id: string; amount: string | number; discount?: string | number | null;
  method: string; forMonth: string | null; paidAt: string; courseName?: string | null;
...
export default function ReceiptPage() {
  ...
  const [p, setP] = useState<Receipt | null>(null);
  const [brand, setBrand] = useState<{ name: string; logo: string }>({ name: "Robocode CRM", logo: "" });
  const [err, setErr] = useState(false);

  useEffect(() => {
    if (!id) return;
    fetch(`/api/payments/${id}`).then(r => r.ok ? r.json() : Promise.reject()).then(setP).catch(() => setErr(true));
    fetch("/api/branding").then(r => r.json()).then(setBrand).catch(() => {});
  }, [id]);

  if (err) return <div className="p-10 text-center text-gray-400">To&apos;lov topilmadi</div>;
  if (!p) return <div className="p-10 text-center text-gray-400">Yuklanmoqda...</div>;
  ...
  return (
    <div className="min-h-screen bg-[#F5F6FA] flex flex-col items-center py-10 px-4 print:bg-white print:py-0" style={{ fontFamily: "'Inter', sans-serif" }}>
      <div className="mb-5 flex gap-2 print:hidden">
        <button
          onClick={() => window.print()}
          className="flex items-center gap-1.5 px-5 py-2.5 bg-[#5E2CA5] hover:bg-[#4a2280] text-white text-[13px] font-semibold rounded-xl shadow-sm transition-colors"
        >
          <Printer className="w-4 h-4" /> Chop etish
        </button>
        <button
          onClick={() => window.close()}
          className="px-5 py-2.5 bg-white border border-gray-200 text-gray-600 text-[13px] font-medium rounded-xl hover:bg-gray-50 transition-colors"
        >
          Yopish
        </button>
      </div>
      ...
      <div id="receipt" className="bg-white w-[340px] p-7 rounded-2xl shadow-lg print:shadow-none print:w-full print:max-w-[340px]">
        <div className="text-center mb-5">
          <div className="inline-flex w-16 h-16 rounded-full bg-[#5E2CA5] items-center justify-center mb-3 overflow-hidden">
            ...
            <img src={brand.logo || "/api/branding/logo"} alt="" className="w-full h-full object-contain" />
          </div>
