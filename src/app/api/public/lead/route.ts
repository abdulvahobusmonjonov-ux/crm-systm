...
// Public form config: active courses + branding (no auth)
export async function GET() {
  try {
    ...
    return NextResponse.json({ courses, name: map["center_name"] || "Robocode CRM", logo: map["brand_logo"] || "" });
  } catch {
    return NextResponse.json({ courses: [], name: "Robocode CRM", logo: "" });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  if (!parsed.success) return NextResponse.json({ error: "Ma'lumotlar to'liq emas" }, { status: 400 });
  const d = parsed.data;
  ...
  if (!admin) return NextResponse.json({ error: "Tizim sozlanmagan" }, { status: 500 });
