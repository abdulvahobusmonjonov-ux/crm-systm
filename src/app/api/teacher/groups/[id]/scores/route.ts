...
// GET ?month=YYYY-MM -> { students, journal: { [date]: { [leadId]: score } } }
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const journal: Record<string, Record<string, number>> = {};
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  for (const r of records) {
    await db.lessonScore.upsert({
      ...
      create: { groupId, leadId: r.leadId, date: d, score: r.score, createdById: session.user.id },
    });
  }
