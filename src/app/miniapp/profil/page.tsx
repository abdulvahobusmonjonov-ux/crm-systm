...
interface ProfileData {
  student: { fullName: string; phone: string; coins: number; courseName: string | null };
  group: {
    name: string; courseName: string | null; teacherName: string | null;
    days: string | null; timeFrom: string | null; timeTo: string | null; room: string | null;
  } | null;
  attendance: {
    total: number; present: number; absent: number; late: number; excused: number;
    percent: number | null; recent: { date: string; status: string }[];
  };
  payments: {
    paidThisMonth: boolean; currentMonth: string; totalPaid: number;
    history: { amount: number; discount: number | null; forMonth: string | null; paidAt: string; type: string }[];
  };
  scores: { average: number | null; list: { examTitle: string; date: string; score: number; maxScore: number }[] };
}
...
export default function MiniAppProfilePage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/telegram/miniapp/profile", {
      method: "POST",
          ...
          <>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl p-4 shadow-sm">
              ...
              {data.group ? (
                <div className="space-y-1.5">
                  ...
                  {data.group.teacherName && (
                    <p className="text-[13px] text-gray-500 dark:text-gray-400">👨‍🏫 {data.group.teacherName}</p>
                  )}
                  {data.group.days && (
                  ...
                  {(data.group.timeFrom || data.group.timeTo) && (
                    <p className="text-[13px] text-gray-500 dark:text-gray-400">
                ...
                <p className="text-[13px] text-gray-400">Hozircha guruhga biriktirilmagansiz.</p>
              )}
            </div>
              ...
              {data.attendance.total > 0 ? (
                <>
                  <div className="flex items-center gap-3 mb-3">
                    <div className="relative w-14 h-14 shrink-0">
                      <svg viewBox="0 0 36 36" className="w-14 h-14 -rotate-90">
                        ...
                        <circle
                          cx="18" cy="18" r="15.5" fill="none" stroke={BRAND} strokeWidth="3"
                          strokeDasharray={`${((data.attendance.percent || 0) / 100) * 97.4} 97.4`}
                          strokeLinecap="round"
                        />
                      </svg>
                  ...
                  <div className="space-y-1 border-t border-gray-100 dark:border-white/5 pt-2">
                    {data.attendance.recent.slice(0, 5).map((a, i) => (
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl p-4 shadow-sm">
              ...
              {data.payments.history.length > 0 && (
                <div className="space-y-1.5 border-t border-gray-100 dark:border-white/5 pt-2">
                  {data.payments.history.slice(0, 5).map((p, i) => (
                    <div key={i} className="flex items-center justify-between text-[12px]">
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl p-4 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                ...
                {data.scores.average !== null && (
                  <span className="text-[13px] font-bold" style={{ color: BRAND }}>O&apos;rtacha: {data.scores.average}</span>
                )}
              </div>
              {data.scores.list.length > 0 ? (
                <div className="space-y-1.5">
                  {data.scores.list.map((s, i) => (
                    <div key={i} className="flex items-center justify-between text-[12px]">
                ...
                <p className="text-[13px] text-gray-400">Hali imtihon natijalari yo&apos;q.</p>
              )}
            </div>
          </>
        )}
