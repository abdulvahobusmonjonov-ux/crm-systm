...
export default function ReceptionPage() {
  ...
  const createLead = async () => {
    if (form.fullName.trim().length < 2) { toast.error("Ismni to'liq kiriting"); return; }
    if (!isPhoneComplete(form.phone)) { toast.error("Telefon raqamni to'liq kiriting"); return; }
    setCreating(true);
  ...
  const logCall = async (leadId: string) => {
    ...
    if (res.ok) toast.success("Qo'ng'iroq qayd etildi");
  ...
  const enrollLead = async (leadId: string) => {
    ...
    if (res.ok) { toast.success("Ro'yxatga olindi"); load(search); }
    else toast.error("Saqlanmadi");
  };
        ...
        <div className="flex items-center gap-2">
          ...
          {canManage && (
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-medium bg-[#5E2CA5] hover:bg-[#4a2280] text-white shadow-sm transition-colors whitespace-nowrap"
            >
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody>
                {leads.map((l) => (
                  <tr key={l.id} className="border-b last:border-0 border-gray-50 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-5 py-3.5">
                      <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full whitespace-nowrap ${LEAD_STATUS_COLORS[l.status as keyof typeof LEAD_STATUS_COLORS] || ""}`}>
                    ...
                    {canManage && (
                      <td className="px-5 py-3.5">
                        <div className="flex items-center justify-end gap-1">
                          <button
                            onClick={() => logCall(l.id)}
                            title="Qo'ng'iroq qayd etish"
                            className="p-1.5 text-gray-400 hover:text-[#5E2CA5] hover:bg-[#5E2CA5]/10 rounded-lg transition-colors"
                          >
                            <Phone className="w-3.5 h-3.5" />
                          </button>
                          {l.status !== "ENROLLED" && (
                            <button
                              onClick={() => enrollLead(l.id)}
